...
/**
 * Tests for the HashEntriesQuery hash_entries collections_grouped_by_size map_collision_ratio extract_list_values
 * hash_set_values
 *
 * @author ktsvetkov
 *
...
public class ExtractCollectionEntriesTest extends ExtractCollectionEntriesBase {
	...
	public void testCustomCollection_Sun_JDK6() throws SnapshotException {
		...
		SnapshotQuery query = SnapshotQuery.parse("hash_entries 0x" + Long.toHexString(objAddress) //$NON-NLS-1$
				+ " -collection java.util.HashMap -array_attribute table -key_attribute key -value_attribute value",
				snapshot);
		IResult result = query.execute(new VoidProgressListener());
		IResultTable table = (IResultTable) result;
		int rowCount = table.getRowCount();

		assert rowCount == numEntries : MessageUtil.format(
				"Expected to extract {0} entries from collection 0x{1} [{2}], but got {3} entries in the result", //$NON-NLS-1$
				numEntries,
				Long.toHexString(objAddress),
	...
	public void testIdentityHashMapCollection_Sun_JDK6() throws SnapshotException {
		...
		SnapshotQuery query = SnapshotQuery.parse("hash_entries " + type, snapshot); //$NON-NLS-1$
		IResult result = query.execute(new VoidProgressListener());
		IResultTable table = (IResultTable) result;
		int rowCount = table.getRowCount();

		assert rowCount == numEntries : MessageUtil.format(
				"Expected to extract {0} entries from collections of type {1} [{2}], but got {3} entries in the result", //$NON-NLS-1$
				numEntries,
				type,
				snapshot.getSnapshotInfo().getPath(),
				rowCount);
