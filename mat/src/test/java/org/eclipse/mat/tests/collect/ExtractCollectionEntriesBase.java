...
public class ExtractCollectionEntriesBase {
	/**
	 * Hamcrest style matcher for regular expression matching. Hamcrest 1.1 doesn't have this.
	 *
	 * @param regex
	...
	void checkListObjects(long objAddress, int numEntries, boolean checkVals, ISnapshot snapshot)
			...
			if (table != null) {
				...
				for (Object o : table.getElements()) {
					...
					if (cv != null) {
						...
						collector.checkThat(prefix + "Should end with number or aAbB",
								cv,
								matchesPattern(".*([0-9]+|[aAbB]+)$"));
					} else {
						collector.checkThat(prefix + "snapshot type is DTFJ-PHD when list item contents is null",
								snapshot.getSnapshotInfo().getProperty("$heapFormat"),
								equalTo((Serializable) "DTFJ-PHD"));
					}
				}
			}
	 ...
	 * @throws SnapshotException
	 */
	protected void checkHashSetObjects(long objAddress, int numEntries, boolean checkVals, ISnapshot snapshot)
			throws SnapshotException {
		IObject obj = snapshot.getObject(snapshot.mapAddressToId(objAddress));
		String prefix = obj.getTechnicalName() + ": ";
		...
		try {
			...
			if (table != null) {
				...
				for (Object o : table.getElements()) {
					IContextObject co = table.getContext(o);
					IClass cls = snapshot.getClassOf(co.getObjectId());
					collector.checkThat(prefix + "hash set should contain Strings",
							cls.getName(),
							equalTo("java.lang.String"));
					...
					if (cv != null) {
						if (checkVals) {
							collector.checkThat(prefix + "Hash set entry should start with the type",
									cv,
									startsWith(name));
						}
						collector.checkThat(prefix + "Should end with number or aAbB",
						...
						collector.checkThat(prefix + "snapshot type is DTFJ-PHD when hash set item contents is null",
								snapshot.getSnapshotInfo().getProperty("$heapFormat"),
								equalTo((Serializable) "DTFJ-PHD"));
					}
				}
	...
	/**
	 * HashMap entries. Check keys and values. Map: 123 : full.class.name:123 Set from map full.class.name:123 : <other>
	 *
	 * @param objAddress
	 * @param numEntries
	 * @param snapshot
	 ...
	 * @param isMap
	 *            whether this is a normal test map, or a map from a set
	 * @throws SnapshotException
	 */
	protected void checkHashEntries(long objAddress,
			int numEntries,
			ISnapshot snapshot,
			boolean checkValueString,
			boolean isMap) throws SnapshotException {
		IObject obj = snapshot.getObject(snapshot.mapAddressToId(objAddress));
		String prefix = obj.getTechnicalName() + ": ";
		...
		try {
			...
			collector.checkThat(prefix + "snapshot type is DTFJ-PHD when unsupported operation for hash entries",
					snapshot.getSnapshotInfo().getProperty("$heapFormat"),
					equalTo((Serializable) "DTFJ-PHD"));
			return;
		}
		...
		collector.checkThat(MessageUtil.format(
				prefix + "Expected to extract {0} entries from collection {1} [{2}], but got {3} entries in the result", //$NON-NLS-1$
				numEntries,
				obj,
				snapshot.getSnapshotInfo().getPath(),
				rowCount), rowCount, equalTo(numEntries));
		...
		for (int i = 0; i < rowCount; ++i) {
			...
			if (k1 != null) {
				...
				if (isMap && checkValueString && k1 instanceof String) {
					collector.checkThat(prefix + "Key should be a number or aAbB",
							(String) k1,
							matchesPattern("[0-9]+|[aAbB]+"));
				}
				if (checkValueString && k1 instanceof String) {
					collector.checkThat(prefix + "Key should end with a number or aAbB",
							(String) k1,
							matchesPattern(".*([0-9]+|[aAbB]+)$"));
				}
				if (!isMap && checkValueString && k1 instanceof String) {
					// Some of the sets are KeySets from Maps so have numeric/aAbB not class names
					collector.checkThat(prefix + "Key contains name of collection class or a number or aAbB",
							(String) k1,
							anyOf(containsString(obj.getClazz().getName()), matchesPattern("[0-9]+|[aAbB]+")));
				}
			}
			if (v1 != null) {
			...
			if (isMap && checkValueString && v1 instanceof String) {
				collector.checkThat(prefix + "Value contains name of collection class",
						(String) v1,
						containsString(obj.getClazz().getName()));
			}
		...
		if (checkValueString && isMap) {
			collector.checkThat(
					MessageUtil.format("Expected some differing keys and values {0} from collection {1} [{2}]",
							obj,
							snapshot.getSnapshotInfo().getPath()),
					diff,
					equalTo(true));
	...
	protected void checkCollectionSize(long objAddress, int numEntries, ISnapshot snapshot) throws SnapshotException {
		IObject obj = snapshot.getObject(snapshot.mapAddressToId(objAddress));
		String prefix = obj.getTechnicalName() + ": ";
		SnapshotQuery query2 =
				SnapshotQuery.parse("collections_grouped_by_size 0x" + Long.toHexString(objAddress), snapshot); //$NON-NLS-1$
	...
	protected void checkList(long objAddress, int numEntries, boolean checkVals, ISnapshot snapshot)
			throws SnapshotException {
		checkCollectionSize(objAddress, numEntries, snapshot);
	 ...
	 * @throws SnapshotException
	 */
	protected void checkCollectionFillRatio(long objAddress, int numEntries, ISnapshot snapshot)
			throws SnapshotException {
		IObject obj = snapshot.getObject(snapshot.mapAddressToId(objAddress));
		String prefix = obj.getTechnicalName() + ": ";
		...
		if (className.equals("java.util.TreeMap") || className.equals("java.util.TreeMap$KeySet")
				...
				|| className.equals("java.util.concurrent.ConcurrentSkipListMap")
				|| className.equals("java.util.concurrent.ConcurrentSkipListMap$KeySet")
				|| className.equals("java.util.concurrent.ConcurrentSkipListMap$EntrySet")
				|| className.equals("java.util.TreeSet")
				|| className.equals("java.util.concurrent.ConcurrentSkipListSet")) {
	 ...
	 * @throws SnapshotException
	 */
	protected void checkMapCollisionRatio(long objAddress, int numEntries, ISnapshot snapshot)
			throws SnapshotException {
		IObject obj = snapshot.getObject(snapshot.mapAddressToId(objAddress));
		String prefix = obj.getTechnicalName() + ": ";
