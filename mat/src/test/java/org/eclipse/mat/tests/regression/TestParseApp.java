	...
	private static class ClockedProgressListener implements IProgressListener {
		...
		public void subTask(String name) {
			...
			if (lastMessage != null) {
				...
				if (p >= 0)
					lastMessage =
							lastMessage.substring(0, p) + "DIR" + lastMessage.substring(p + absolutePath.length());
				...
				if (showMem) {
					...
					out.println(String.format("Task: [%02d] %s %s ms used %d free %d total %d max %d",
							++count,
							lastMessage,
					...
					out.println(String
							.format("Task: [%02d] %s %s ms", ++count, lastMessage, String.valueOf(now - timestamp)));
				}
			}
