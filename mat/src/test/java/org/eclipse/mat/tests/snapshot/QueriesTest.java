...
public class QueriesTest {
	private static final String OQL_MIXED_RESULT =
			"oql \"SELECT s.value FROM java.lang.Integer s" + " UNION (SELECT s.value FROM java.lang.Character s )" + // No
																														// Characters
																														// in
																														// snapshot
																														// so
	...
	/**
	 * Test for grouping dominator tree by class loader Enable test when fix is available
	 *
					...
					try {
						...
						if (prev instanceof Number) {
							if (val instanceof Number) {
								// Numbers in order
								int compare =
										Double.compare(((Number) prev).doubleValue(), ((Number) val).doubleValue());
								assertTrue(compare <= 0);
							} else {
	...
	public void testCustomRetainedSet() throws SnapshotException {
		SnapshotQuery query = SnapshotQuery.parse(
				"customized_retained_set -x java.lang.ref.WeakReference:referent java.lang.ref.SoftReference:referent; 0x2ca48ee8",
				snapshot);
	...
	public void testFindStrings2() throws SnapshotException {
		// 3 object arguments
		SnapshotQuery query = SnapshotQuery.parse(
				"find_strings select * from java.lang.String; java.lang.String select * from java.lang.String; -pattern p0",
				snapshot);
		CheckedWorkProgressListener listener = new CheckedWorkProgressListener(collector);
		IResultTree t = (IResultTree) query.execute(listener);
		assertTrue(t != null);
	...
	public void testExportHprofReport() throws SnapshotException, IOException {
		...
		try {
			SnapshotQuery query = SnapshotQuery
					.parse("default_report org.eclipse.mat.hprof:export -params output=" + fn.getPath(), snapshot);
			IResult t = query.execute(new CheckedProgressListener(collector));
			assertNotNull(t);
			ISnapshot newSnapshot = SnapshotFactory
					.openSnapshot(fn, Collections.<String, String>emptyMap(), new CheckedProgressListener(collector));
	...
	public void testExportHprofReportFile() throws SnapshotException, IOException {
		...
		try {
			...
			try {
				SnapshotQuery query = SnapshotQuery
						.parse("create_report " + rep.getPath() + " -params output=" + fn.getPath(), snapshot);
				IResult t = query.execute(new CheckedProgressListener(collector));
				assertNotNull(t);
				ISnapshot newSnapshot = SnapshotFactory.openSnapshot(fn,
	...
	public void testCompareQuery() throws SnapshotException, IOException {
		...
		SnapshotQuery query =
				SnapshotQuery.parse("delta_histogram -snapshot2 " + snapshot2.getSnapshotInfo().getPath(), snapshot);
		IResult t = query.execute(new CheckedProgressListener(collector));
		assertNotNull(t);
		int intplus = 0;
		int intminus = 0;
	...
	public void testCompareReport() throws SnapshotException, IOException {
		ISnapshot snapshot2 = TestSnapshots.getSnapshot(TestSnapshots.SUN_JDK6_18_64BIT, false); // Do not dispose this
																									// as shared
		SnapshotQuery query = SnapshotQuery.parse(
				"default_report org.eclipse.mat.api:compare -params snapshot2=" + snapshot2.getSnapshotInfo().getPath(),
				snapshot);
		IResult t = query.execute(new CheckedProgressListener(collector));
		assertNotNull(t);
	...
	public void testOverview2Report() throws SnapshotException, IOException {
		ISnapshot snapshot2 = TestSnapshots.getSnapshot(TestSnapshots.SUN_JDK6_18_64BIT, false); // Do not dispose this
																									// as shared
		SnapshotQuery query = SnapshotQuery.parse(
				"default_report org.eclipse.mat.api:overview2 -params baseline=" + snapshot.getSnapshotInfo().getPath(),
				snapshot2);
		IResult t = query.execute(new CheckedProgressListener(collector));
		assertNotNull(t);
		if (t instanceof DisplayFileResult) {
	...
	public void testSuspects2Report() throws SnapshotException, IOException {
		ISnapshot snapshot2 = TestSnapshots.getSnapshot(TestSnapshots.SUN_JDK6_18_64BIT, false); // Do not dispose this
																									// as shared
		SnapshotQuery query = SnapshotQuery.parse(
				"default_report org.eclipse.mat.api:suspects2 -params baseline=" + snapshot.getSnapshotInfo().getPath(),
				snapshot2);
		IResult t = query.execute(new CheckedProgressListener(collector));
		assertNotNull(t);
	}
	...
	public void testLeakHunter2ReportIBM8_7() throws SnapshotException, IOException {
		ISnapshot snapshot1 = TestSnapshots.getSnapshot(TestSnapshots.IBM_JDK7_64BIT_SYSTEM, false); // Do not dispose
																										// this as
																										// shared
		ISnapshot snapshot2 = TestSnapshots.getSnapshot(TestSnapshots.IBM_JDK8_64BIT_SYSTEM, false); // Do not dispose
																										// this as
																										// shared
	...
	public void testLeakHunter2ReportJDK11() throws SnapshotException, IOException {
		ISnapshot snapshot1 = TestSnapshots.getSnapshot(TestSnapshots.ADOPTOPENJDK_HOTSPOT_JDK11_0_4_11_64BIT, false); // Do
																														// not
																														// dispose
																														// this
																														// as
																														// shared
		ISnapshot snapshot2 = TestSnapshots.getSnapshot(TestSnapshots.OPENJDK_JDK11_04_64BIT, false); // Do not dispose
																										// this as
																										// shared
	...
	public void testLeakHunter2Report(ISnapshot snapshot1,
			ISnapshot snapshot2,
			int expectedProbs,
			int expectedDomTree,
			int expectedSubCommands) throws SnapshotException, IOException {
		SnapshotQuery query =
				SnapshotQuery.parse("leakhunter2 -baseline " + snapshot1.getSnapshotInfo().getPath(), snapshot2);
		IResult t = query.execute(new CheckedProgressListener(collector));
		assertNotNull(t);
		if (expectedProbs == 0) {
	...
	public void testCompareThreads1() throws SnapshotException {
		...
		SnapshotQuery query3 = SnapshotQuery.parse(
				"comparetablesquery -mode ABSOLUTE -prefix -mask \"\\s@ 0x[0-9a-f]+|^(\\[[0-9]+\\], )*\\[[0-9]+\\]$|(?<=\\p{javaJavaIdentifierPart}\\[)\\d+(?=\\])\" -x java.util.HashMap$Node:key java.util.Hashtable$Entry:key java.util.WeakHashMap$Entry:referent java.util.concurrent.ConcurrentHashMap$Node:key",
				snapshot2);
		...
		assertThat(tree.getElements().size(),
				equalTo(Math.max(tree1.getElements().size(), tree2.getElements().size())));
		int same = 0;
	...
	public void testQueryReport1() throws SnapshotException, IOException {
		SnapshotQuery query =
				SnapshotQuery.parse("default_report org.eclipse.mat.api:query -params command=histogram", snapshot);
		IResult t = query.execute(new CheckedProgressListener(collector));
		assertNotNull(t);
	...
	public void testQueryReport2() throws SnapshotException, IOException {
		SnapshotQuery query = SnapshotQuery.parse(
				"default_report org.eclipse.mat.api:query -params \"command=oql \\\"select * from java.lang.String\\\"\"",
				snapshot);
		IResult t = query.execute(new CheckedProgressListener(collector));
		assertNotNull(t);
		assertThat(t, instanceOf(DisplayFileResult.class));
