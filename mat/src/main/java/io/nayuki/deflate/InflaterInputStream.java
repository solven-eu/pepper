...
public final class InflaterInputStream extends FilterInputStream {
	...
	/*
	 * For marking. markPos >= 0 is position of mark in buffer [dictionary] markPos = -1 not set markPos = -2 has been
	 * set, but nothing has been read markPos = -3 has been set but too much has been read
	 */
	...
	/**
	 * Constructs an inflater input stream over the specified underlying input stream, and with the specified option for
	 * detachability. The underlying stream must contain DEFLATE-compressed data with no headers or footers (e.g. must
	 * be unwrapped from the zlib or gzip container formats). Detachability allows {@link #detach()} to be called, and
	 * requires the specified input stream to support marking.
	 *
	 * @param in
	 *            the underlying input stream of raw DEFLATE-compressed data
	 * @param detachable
	 *            whether {@code detach()} can be called later
	 * @throws NullPointerException
	 *             if the input stream is {@code null}
	 * @throws IllegalArgumentException
	 *             if {@code detach == true} but {@code in.markSupported() == false}
	 */
	...
	/**
	 * Constructs an inflater input stream over the specified underlying input stream, with the specified options for
	 * detachability and input buffer size. The underlying stream must contain DEFLATE-compressed data with no headers
	 * or footers (e.g. must be unwrapped from the zlib or gzip container formats). Detachability allows
	 * {@link #detach()} to be called, and requires the specified input stream to support marking.
	 *
	 * @param in
	 *            the underlying input stream of raw DEFLATE-compressed data
	 * @param detachable
	 *            whether {@code detach()} can be called later
	 * @param inBufLen
	 *            the size of the internal read buffer, which must be positive
	 * @throws NullPointerException
	 *             if the input stream is {@code null}
	 * @throws IllegalArgumentException
	 *             if {@code inBufLen < 1}
	 ...
	 *
	 * Only safe to use copy or original once the underlying stream has been positioned to the appropriate location.
	...
	/**
	 * Reads the next byte of decompressed data from this stream. If data is available then a number in the range [0,
	 * 255] is returned (blocking if necessary); otherwise &minus;1 is returned if the end of stream is reached.
	 *
	 * @return the next unsigned byte of data, or &minus;1 for the end of stream
	 * @throws IOException
	 *             if an I/O exception occurred in the underlying input stream, the end of stream was encountered at an
	 *             unexpected position, or the compressed data has a format error
	...
	/**
	 * Reads some bytes from the decompressed data of this stream into the specified array's subrange. This returns the
	 * number of data bytes that were stored into the array, and is in the range [&minus;1, len]. Note that 0 can be
	 * returned even if the end of stream hasn't been reached yet.
	 *
	 * @throws NullPointerException
	 *             if the array is {@code null}
	 * @throws ArrayIndexOutOfBoundsException
	 *             if the array subrange is out of bounds
	 * @throws IOException
	 *             if an I/O exception occurred in the underlying input stream, the end of stream was encountered at an
	 *             unexpected position, or the compressed data has a format error
	 * @throws IllegalStateException
	 ...
	 * This uses the dictionary array as a cache of the last bytes returned to the caller. The dictionary is 32768 bytes
	 * in size, however when reading compressed data the inflator can generate an extra 257 bytes which are not returned
	 * to the caller but are stored in the dictionary. There is therefore a limit of 32768 - 257 = 32211 bytes that can
	 * be stored. Strictly speaking, the InputStream contract expects any value of limit to be honoured, and for
	 * implementation to add extra buffering to achieve this. Addition for Eclipse MAT.
	 *
	 * @param limit
	 *            the caller can read at least this many bytes before calling {@link #reset()}
	 * @see #reset()
	 */
	@Override
	...
	/**
	 * Goes back in the output to the point where {@link #mark(int)} was called. Addition for Eclipse MAT
	 *
	 * @throws IOException
	 *             if it is not possible to go back to the mark point. The current position is then unchanged.
	 * @see #mark(int)
	 */
	@Override
	...
	/**
	 * Does this stream support {@link #mark(int)} and {@link #reset()} ? It does. Addition for Eclipse MAT.
	 *
	 * @return true
	 * @see #mark(int)
	 * @see #reset()
	...
	/**
	 * The number of bytes which can be read without blocking. With the underlying stream it is not clear how many bytes
	 * those will expand to so just rely on what is in the buffers. Addition for Eclipse MAT.
	 *
	...
	/**
	 * See if too much data has been read for a {@link #reset()} to not work anymore. Addition for Eclipse MAT.
	 *
	 * @param dictionaryIndex
	 *            the next place to store in the dictionary
	 * @param count
	 *            the number of bytes to add to dictionary
	 */
		...
		while (result < len) {
			...
			if (inputBitBufferLength < 48) {
				...
				if (numBytes == 2) { // Only implement special cases that occur frequently in practice
					...
					inputBitBuffer |= (long) ((c[i] & 0xFF) | (c[i + 1] & 0xFF) << 8
							| (c[i + 2] & 0xFF) << 16) << inputBitBufferLength;
					inputBitBufferLength += 3 * 8;
					inputBufferIndex += 3;
				} else if (numBytes == 4) {
					inputBitBuffer |=
							(((c[i] & 0xFF) | (c[i + 1] & 0xFF) << 8 | (c[i + 2] & 0xFF) << 16 | c[i + 3] << 24)
									& 0xFFFFFFFFL) << inputBitBufferLength;
	...
	/**
	 * Detaches the underlying input stream from this decompressor. This puts the underlying stream at the position of
	 * the first byte after the data that this decompressor actually consumed. Calling {@code detach()} invalidates this
	 * stream object but doesn't close the underlying stream.
	 * <p>
	 * This method exists because for efficiency, the decompressor may read more bytes from the underlying stream than
	 * necessary to produce the decompressed data. If you want to continue reading the underlying stream exactly after
	 * the point the DEFLATE-compressed data ends, then it is necessary to call this detach method.
	 * </p>
	 * <p>
	 * This can only be called once, and is mutually exclusive with respect to calling {@link #close()}. It is illegal
	 * to call {@link #read()} after detaching.
	 * </p>
	 *
	 * @throws IllegalStateException
	 *             if detach was already called or this stream has been closed
	 * @throws IOException
	...
	/**
	 * Closes this input stream and the underlying stream. It is illegal to call {@link #read()} or {@link #detach()}
	 * after closing. It is idempotent to call this {@link #close()} method more than once.
	 *
	 * @throws IOException
	...
	/*
	 * Converts the given array of symbol code lengths into a canonical code tree. A symbol code length is either zero
	 * (absent from the tree) or a positive integer.
	 *
	 * A code tree is an array of integers, where each pair represents a node. Each pair is adjacent and starts on an
	 * even index. The first element of the pair represents the left child and the second element represents the right
	 * child. The root node is at index 0. If an element is non-negative, then it is the index of the child node in the
	 * array. Otherwise it is the bitwise complement of the leaf symbol. This tree is used in decodeSymbol() and
	 * codeTreeToCodeTable(). Not every element of the array needs to be used, nor do used elements need to be
	 * contiguous.
	 *
	 * For example, this Huffman tree: o / \ o \ / \ \ 'a' 'b' 'c' is serialized as this array: {2, ~'c', ~'a', ~'b'}
	 * because the root is located at index 0 and the other internal node is located at index 2.
	 */
	...
	/*
	 * Converts a code tree array into a fast look-up table that consumes up to CODE_TABLE_BITS at once. Each entry i in
	 * the table encodes the result of decoding starting from the root and consuming the bits of i starting from the
	 * lowest-order bits.
	 *
	 * Each array element encodes (numBitsConsumed << 11) | (node & 0x7FF), where: - numBitsConsumed is a 4-bit unsigned
	 * integer in the range [1, CODE_TABLE_BITS]. - node is an 11-bit signed integer representing either the current
	 * node (which is a non-negative number) after consuming all the available bits from i, or the bitwise complement of
	 * the decoded symbol (so it's negative). Note that each element is a non-negative number.
	 */
	...
	private void readBytes(byte[] b, int off, int len) throws IOException {
		...
		if (inputBitBufferLength < 0 || inputBitBufferLength > 63 || inputBitBuffer >>> inputBitBufferLength != 0)
	...
	private static final short[] RUN_LENGTH_TABLE = { 24,
			32,
			40,
			48,
			56,
	...
	private static final int[] DISTANCE_TABLE = { 16,
			32,
			48,
			64,
			81,
