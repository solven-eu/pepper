...
public class WrapperCollectionExtractor implements ICollectionExtractor {
	...
	protected ExtractedCollection extractList(IObject coll) {
		...
		if (ec instanceof ExtractedCollection)
			...
			throw new UnsupportedOperationException(desc(Messages.WrapperCollectionExtractor_NotAList1,
					Messages.WrapperCollectionExtractor_NotAList2,
					coll,
					ec));
	}
	...
	protected ExtractedMap extractMap(IObject coll) {
		...
		if (ec instanceof ExtractedMap)
			...
			throw new UnsupportedOperationException(desc(Messages.WrapperCollectionExtractor_NotAMap1,
					Messages.WrapperCollectionExtractor_NotAMap2,
					coll,
					ec));
	}
	...
	private AbstractExtractedCollection<?, ?> guessWrappedFromOutbounds(IObject coll) throws SnapshotException {
		...
		for (int outId : outbounds) {
			/*
			 * SynchronizedMap has a reference to itself for synchronizing on, so skip that.
			 */
			if (outId == coll.getObjectId())
				continue;
			AbstractExtractedCollection<?, ?> ex =
					CollectionExtractionUtils.extractCollection(snapshot.getObject(outId));
			if (ex != null) {
				if (extracted == null) {
					extracted = ex;
				} else {
					throw new IllegalArgumentException(
							MessageUtil.format(Messages.WrapperCollectionExtractor_CouldNotResolveMultiple,
									field,
									coll.getTechnicalName()));
				}
			}
		}
		...
		if (extracted != null) {
			...
			throw new IllegalArgumentException(MessageUtil
					.format(Messages.WrapperCollectionExtractor_CouldNotResolveNone, field, coll.getTechnicalName()));
		}
	}
}
