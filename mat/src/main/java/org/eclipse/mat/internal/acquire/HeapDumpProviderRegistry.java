...
public class HeapDumpProviderRegistry extends RegistryReader<IHeapDumpProvider> {
	private final Map<String, HeapDumpProviderDescriptor> providersByIdentifier =
			new HashMap<String, HeapDumpProviderDescriptor>();
	private final Map<String, HeapDumpProviderDescriptor> providersByClass =
			new HashMap<String, HeapDumpProviderDescriptor>();
	...
	protected IHeapDumpProvider createDelegate(IConfigurationElement configElement) throws CoreException {
		try {
			...
			if (MATPlugin.getDefault().isDebugging())
				MATPlugin.log(IStatus.INFO,
						MessageUtil.format("IHeapDumpProvider ''{0}'' registered.", descriptor.getName())); //$NON-NLS-1$

			return descriptor != null ? dumpProvider : null;
		} catch (SnapshotException e) {
			throw new CoreException(new Status(IStatus.ERROR,
					MATPlugin.PLUGIN_ID,
					MessageUtil.format("Error registering heap dump adapter: {0}", configElement.getAttribute("impl")), //$NON-NLS-1$ //$NON-NLS-2$
					e));
		}
	}
	...
	public synchronized HeapDumpProviderDescriptor getHeapDumpProvider(
			Class<? extends IHeapDumpProvider> providerClass) {
	 ...
	 */
	private final synchronized HeapDumpProviderDescriptor registerProvider(IHeapDumpProvider provider)
			throws SnapshotException {
		...
		if (providersByIdentifier.containsKey(identifier))
			throw new SnapshotException(MessageUtil.format(Messages.HeapDumpProviderRegistry_NameAlreadyBouneErrorMsg,
					identifier,
					providersByIdentifier.get(identifier).getSubject().getName()));
		...
		HeapDumpProviderDescriptor descriptor =
				new HeapDumpProviderDescriptor(identifier, name, usage, help, helpUrl, helpLoc, provider);
	...
	private void addArguments(IHeapDumpProvider provider,
			Class<?> clazz,
			HeapDumpProviderDescriptor descriptor,
			ResourceBundle i18n) throws SnapshotException {
		...
		for (Field field : fields) {
			try {
				...
				if (argument != null) {
					...
					String help = translate(i18n,
							clazz.getSimpleName() + "." + argDescriptor.getName() + ".help", //$NON-NLS-1$//$NON-NLS-2$
							h != null ? h.value() : null);
					if (help != null)
				...
				throw new SnapshotException(
						MessageUtil.format(Messages.HeapDumpProviderRegistry_ErrorGettingArgumentErrorMsg,
								field.getName(),
								clazz.getName()),
						e);
			}
	...
	private ArgumentDescriptor fromAnnotation(Class<?> clazz, Argument annotation, Field field, Object defaultValue)
		...
		if (advice == Argument.Advice.CLASS_NAME_PATTERN && !Pattern.class.isAssignableFrom(d.getType())) {
			String msg = MessageUtil.format(Messages.HeapDumpProviderRegistry_WrongTypeErrorMsg,
					field.getName(),
					clazz.getName(),
					Argument.Advice.CLASS_NAME_PATTERN,
