...
public final class HeapObjectParamArgument extends HeapObjectArgumentFactory {
	...
	private static class IteratorImpl implements Iterator<int[]> {
			...
			try {
				...
				if (argument instanceof Pattern) {
					...
					throw new SnapshotException(
							MessageUtil.format(Messages.HeapObjectParamArgument_ErrorMsg_UnknownArgument,
		...
		private int[] collectObjectsByClass(IClass clazz) throws SnapshotException {
			...
			if (hopa.isVerbose) {
				logger.log(Level.INFO,
						MessageUtil.format(Messages.HeapObjectParamArgument_Msg_AddedInstances,
								clazz.getName(),
								answer.length));
			}
		...
		private int[] collectObjectsByPattern(Pattern pattern) throws SnapshotException {
			if (hopa.isVerbose) {
				logger.log(Level.INFO,
						MessageUtil.format(Messages.HeapObjectParamArgument_Msg_SearchingByPattern,
								pattern.toString()));
			}
			...
			if (hopa.isVerbose) {
				...
				logger.log(Level.INFO,
						MessageUtil.format(Messages.HeapObjectParamArgument_Msg_MatchingPattern,
								classesByName.size(),
								instanceCount));
			}
		...
		private int[] collectObjectsByAddress(long address) throws SnapshotException {
			...
			if (objectId < 0)
				throw new SnapshotException(MessageUtil.format(Messages.HeapObjectParamArgument_Msg_ObjectFound,
		...
		private int[] collectObjectsByOQL(String queryString) throws OQLParseException, SnapshotException {
			...
			if (result instanceof int[]) {
				...
				throw new SnapshotException(
						MessageUtil.format(Messages.HeapObjectParamArgument_ErrorMsg_NorResult, queryString));
			} else {
				throw new SnapshotException(
						MessageUtil.format(Messages.HeapObjectParamArgument_ErrorMsg_NotAListOfObjects, queryString));
			}
		}
		...
		private int[] evalLoaderInstances(int[] answer) throws SnapshotException {
			...
			for (int ii = 0; ii < answer.length; ii++) {
				...
				if (!(classLoader instanceof IClassLoader)) {
					...
					throw new SnapshotException(MessageUtil.format(msg,
							Long.toHexString(classLoader.getObjectAddress()),
							objectId,
							classLoader.getClazz().getName(),
