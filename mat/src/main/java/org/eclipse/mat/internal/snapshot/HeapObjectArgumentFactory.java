...
public abstract class HeapObjectArgumentFactory implements ArgumentFactory {
	...
	public Object build(ArgumentDescriptor argument) throws SnapshotException {
		try {
			...
			throw new SnapshotException(MessageUtil.format(Messages.HeapObjectArgumentFactory_ErrorMsg_SettingField,
	...
	private Object assignObjectIds(ArgumentDescriptor argument, ISnapshot snapshot, int[] objectIds)
		...
		if (argument.isArray()) {
			...
			if (argument.getType() == Integer.class) {
				...
				throw new SnapshotException(
						MessageUtil.format(Messages.HeapObjectArgumentFactory_ErrorMsg_TypeNotSupported,
			...
			if (objectIds.length != 1)
				throw new SnapshotException(
						MessageUtil.format(Messages.HeapObjectArgumentFactory_ErrorMsg_MultipleObjects,
								argument.getName(),
								objectIds.length));
