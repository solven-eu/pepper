...
public class MultiplePath2GCRootsQuery implements IQuery {
	public enum Grouping {
		...
		FROM_GC_ROOTS_BY_CLASS(Messages.MultiplePath2GCRootsQuery_Group_FromGCRootsOnClass,
				Icons.getURL("mpaths_from_gc_by_class.gif")), //$NON-NLS-1$
		FROM_OBJECTS_BY_CLASS(Messages.MultiplePath2GCRootsQuery_Group_ToGCRoots,
				Icons.getURL("mpaths_to_gc_by_class.gif")); //$NON-NLS-1$
	...
	public IResult execute(IProgressListener listener) throws Exception {
		...
		IMultiplePathsFromGCRootsComputer computer =
				snapshot.getMultiplePathsFromGCRoots(objects.getIds(listener), excludeMap);
	...
	public static Tree create(ISnapshot snapshot,
			IMultiplePathsFromGCRootsComputer computer,
			int[] selection,
		...
		return selection != null ? new TreeByObjectSelected(snapshot, result, selection)
				: new TreeByObject(snapshot, result);
	}
	 ...
	 * @param computer
	 * @param selection
	 *            list of classes, or null, which are the path to be expanded.
	...
	public static Tree create(ISnapshot snapshot,
		...
		return selection != null ? new TreeByClassSelected(snapshot, result, selection, mergeFromRoots)
				: new TreeByClass(snapshot, result, mergeFromRoots);
	}
	...
	/* package */static class TreeByObject extends Tree implements IIconProvider, IDecorator {
		...
		public URL getIcon(Object row) {
			Node n = (Node) row;
			return hasChildren(row) ? Icons.outbound(snapshot, n.objectId) : Icons.forObject(snapshot, n.objectId);
		}
