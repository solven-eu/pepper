...
public class ExtractionUtils {
	...
	public static JdkVersion resolveVersion(ISnapshot snapshot) throws SnapshotException {
				...
				if (jreIndex != -1) {
					...
					if (jreVersion.length() >= 3) {
						...
						if (jreVersion.equals("9") || jreVersion.startsWith("9."))
							...
							if (jvmInfo.matches(".*70sr.*\\(SR[1-3][^0-9].*")
									|| jvmInfo.matches(".*\\(GA") && !jvmInfo.matches(".*70_27.*")) {
		...
		if ((classes = snapshot.getClassesByName("com.ibm.misc.JavaRuntimeVersion", false)) != null //$NON-NLS-1$
				&& !classes.isEmpty())
			return IBM15;
		else if ((classes = snapshot.getClassesByName("com.ibm.oti.vm.BootstrapClassLoader", false)) != null
				&& !classes.isEmpty()) {
			// com.ibm.oti.util.Msg
			if ((classes = snapshot.getClassesByName("com.ibm.oti.util.Msg", false)) != null && !classes.isEmpty())
				return IBM19;
			// java.lang.Integer$IntegerCache java 8, com/ibm/oti/util/WeakReferenceNode
			if ((classes = snapshot.getClassesByName("com/ibm/oti/util/WeakReferenceNode", false)) != null
					&& !classes.isEmpty())
				return IBM18;
			// com.ibm.oti.vm.VMLangAccess java 7
			if ((classes = snapshot.getClassesByName("com.ibm.oti.vm.VMLangAccess", false)) != null
					&& !classes.isEmpty())
				return IBM17;
			return IBM16; // $NON-NLS-1$
		} else if ((classes = snapshot.getClassesByName("com.ibm.jvm.Trace", false)) != null && !classes.isEmpty()) //$NON-NLS-1$
			return IBM14;
		...
		if ((classes = snapshot.getClassesByName("java.lang.Module", false)) != null && !classes.isEmpty())
			return JAVA19;
		if ((classes = snapshot.getClassesByName("java.util.Spliterators", false)) != null && !classes.isEmpty())
			return JAVA18;
		return SUN;
	}
}
