...
public abstract class HashedMapCollectionExtractorBase extends MapCollectionExtractorBase {
	...
	protected IObjectArray extractBackingArray(IObject coll) throws SnapshotException {
		...
		if (obj instanceof IObjectArray) {
			...
			String msg = MessageUtil
					.format(Messages.CollectionUtil_BadBackingArray, arrayField, coll.getTechnicalName(), desc);
	...
	/**
	 * Get the only object field from the object which is of the same type as the source
	 *
	 * @param sourceId
	 * @param seen
	 *            whether seen yet
