...
public abstract class RegistryReader<D> implements IExtensionChangeHandler {
	...
	protected final void init(IExtensionTracker tracker, String extensionPointIdentifier) {
		final IExtensionPoint extensionPoint =
				Platform.getExtensionRegistry().getExtensionPoint(extensionPointIdentifier);
		if (tracker != null)
		...
		for (int i = 0; i < configs.length; ++i) {
			...
			try {
				...
				Logger.getLogger(getClass().getName())
						.log(Level.SEVERE, MessageUtil.format(Messages.RegistryReader_Error_Registry, configs[i]), e);
			}
