...
public final class FileUtils {
	...
	private static class DirDeleter extends Thread {
		...
		private void deleteDirectory(File dir) {
				...
				for (int i = 0; i < fileArray.length; i++) {
					if (fileArray[i].isDirectory())
						...
						ReportPlugin.log(IStatus.WARNING,
								MessageUtil.format(Messages.FileUtils_FileUtils_FailedToDeleteFile,
			...
			if (!dir.delete()) {
				ReportPlugin.log(IStatus.WARNING,
						MessageUtil.format(Messages.FileUtils_FailedToDeleteDirectory, dir.getAbsolutePath()));
			}
		}
	}

	/**
	 * Unzip the specified zip file into a directory of the same name as the zip file minus the suffix within the same
	 * parent directory.
	 *
	 * @param file
	 *            .zip file
