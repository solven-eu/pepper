 ...
 * <ul>
 * <li>if the pattern does not contain one of the expressions .* !^ (at the beginning) $ (at the end), then a .* is
 * added at the beginning and at the end of the pattern
 * <li>if the pattern contains [], it is replaced by \[\]
 * <li>if the pattern contains $ not at the end (inner classes), it is replaced by \$
 * </ul>
 */
public class PatternUtil {
	/**
	 * Fix up a pattern to be a true regular expression pattern. Add dots and starts at the beginning and end if not
	 * already there.
	 *
	 * @param pattern
	 *            the pattern used for matching class names
	 ...
	 *
	 * @param pattern
	 *            the almost RegEx pattern
	 * @param addDotStars
	 *            if true then if the pattern does not contain one of the expressions .* !^ (at the beginning) $ (at the
	 *            end), then a .* is added at the beginning and at the end of the pattern.
	...
	public static String smartFix(String pattern, boolean addDotStars) {
		if (pattern == null || pattern.trim().length() == 0) {
			...
			if (addDotStars) {
				...
				if (!(answer.indexOf(".*") >= 0 || answer.charAt(0) == '^'
