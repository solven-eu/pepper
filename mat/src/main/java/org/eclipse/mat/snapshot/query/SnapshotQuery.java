 ...
 *
 * <p>
 * NOTE: The queries delivered with Memory Analyzer and their expected parameters are not part of the API. Both
 * names/identifiers and parameters may change.
 * </p>
 *
 * Usage example:
 *
...
public class SnapshotQuery {
	...
	private static void checkSubjects(String name,
			ISnapshot snapshot,
			QueryDescriptor query,
			IQueryContext queryContext) throws SnapshotException {
		if (unsuitableSubjects(query, queryContext)) {
			throw new SnapshotException(MessageUtil.format(Messages.SnapshotQuery_ErrorMsg_UnsuitableSubjects,
					name,
					Arrays.asList(extractSubjects(query))));
		}
	}
	...
	/**
	 * Factory method to create a query by command line, i.e. setting the arguments accordingly.
	 */
	...
	/**
	 * Set the argument identified by <code>name</code>. Heap objects can be provided as <code>int</code>,
	 * <code>Integer</code>, <code>int[]</code> or <code>IObject</code>.
	 *
	 * @param name
	...
	public SnapshotQuery setArgument(String name, Object value) throws SnapshotException {
		...
		if (argument == null)
			throw new SnapshotException(
					MessageUtil.format(Messages.SnapshotQuery_ErrorMsg_UnkownArgument, name, query.getIdentifier()));
		...
		if ((argument.getType() == int.class && argument.getAdvice() == Argument.Advice.HEAP_OBJECT) //
			...
			if (value instanceof IObject) {
				...
				throw new SnapshotException(MessageUtil
						.format(Messages.SnapshotQuery_ErrorMsg_UnsupportedTyp, name, value.getClass().getName()));
			}
		}
	...
	/**
	 * Execute the query and return a {@link RefinedResultBuilder} which allows for filtering, sorting and limiting of
	 * the result.
	 */
	public RefinedResultBuilder refine(IProgressListener listener) throws SnapshotException {
		IResult result = execute(listener);
		if (result == null)
			throw new SnapshotException(MessageUtil.format(Messages.SnapshotQuery_ErrorMsg_NoResult,
					arguments.getQueryDescriptor().getIdentifier()));
