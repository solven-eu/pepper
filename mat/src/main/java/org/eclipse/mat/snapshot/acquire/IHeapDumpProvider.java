...
/**
 * Provides functionality to acquire a heap dump from a locally running Java process Implementations of this interface
 * need to be registered using the <code>org.eclipse.mat.api.heapDumpProvider</code> extension point. Arguments can be
 * injected into the query using public fields marked with the {@link Argument} annotation. Typical arguments to be
 * supplied by the user of the heap dump provider include
 * <ul>
 * <li>boolean flags</li>
 * <li>int parm</li>
...
public interface IHeapDumpProvider {
	/**
	 * Returns a list of locally running Java processes from which the heap dump provider can attempt to acquire a heap
	 * dump
	 *
	 * @param listener
	 *            a progress listener
	 ...
	 *            a preferred filename under which the heap dump should be saved. The {@link IHeapDumpProvider} is not
	 *            obliged to provide the heap dump at this location
	 * @param listener
	 *            a progress listener
	 * @return the File under which the successfully generated heap dump is saved
	 * @throws SnapshotException
	 */
