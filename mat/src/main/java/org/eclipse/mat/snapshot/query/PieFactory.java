...
public final class PieFactory {
	...
	/**
	 * Create a pie factory for the given snapshot. The size of the pie is the total heap size
	...
	/**
	 * Create a pie factory for objects. Objects must be added either via {@link #addSlice(IObject)} or
	 * {@link #addSlice(int, String, long, long)} methods.
	 *
	 * @param pieSize
	 *            total size of the pie
	 */
	 ...
	 * <p>
	 * To use this method, one needs to pass a {@link ISnapshot} to the constructor.
	 *
	 * @param objectId
	 *            object id
	 * @return a new slice
	...
	public Slice addSlice(IObject object, Color color) {
		return addSlice(object
				.getObjectId(), object.getDisplayName(), object.getUsedHeapSize(), object.getRetainedHeapSize(), color);
	}
	...
	private final static class SliceImpl implements IResultPie.ColoredSlice, Serializable {
		 ...
		 * {@link org.eclipse.ui.forms.widgets.FormText} which expects an XML, not HTML type input, so &lt;br/&gt; must
		 * be closed.
		 */
