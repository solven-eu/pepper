...
public class RetainedSizeDerivedData extends ContextDerivedData {
	...
	/**
	 * Get the label for the extra column. Based on the column name plus information from the provider as to name of the
	 * set of objects.
	 */
	@Override
	public String labelFor(DerivedColumn derivedColumn, ContextProvider provider) {
		return provider.getLabel() == null ? derivedColumn.getLabel()
				: derivedColumn.getLabel() + " - " //$NON-NLS-1$
						+ provider.getLabel();
	}
		...
		public void calculate(DerivedOperation operation, Object row, IProgressListener listener)
			...
			if (contextObject instanceof IContextObjectSet) {
				...
				if (objectId < 0) {
					...
					Logger.getLogger(getClass().getName())
							.log(Level.SEVERE, MessageUtil.format(msg, provider.getClass().getName(), row.toString()));
				} else {
	...
	private static class AllClasses extends DerivedCalculatorImpl {
		...
		public void calculate(DerivedOperation operation, Object row, IProgressListener listener)
			...
			if (row instanceof ClassHistogramRecord) {
				...
				((ClassLoaderHistogramRecord) row)
						.calculateRetainedSize(snapshot, true, operation == APPROXIMATE, listener);
			} else {
	...
	private static final class RetainedSizeFormat extends BytesFormat {
		...
		public Object parseObject(String source, ParsePosition pos) {
			Object ret;
			if (source.regionMatches(pos.getIndex(),
					Messages.RetainedSizeDerivedData_Approximate,
					0,
					Messages.RetainedSizeDerivedData_Approximate.length())) {
				int pi = pos.getIndex();
