...
public final class TQuantize {
	...
	/**
	 * Creates a {@link TQuantize} object which aggregates the table by the value of the columns.
	 */
	...
	/**
	 * Creates a {@link TQuantize} object which aggregates the table by one of the well-known targets, e.g. by class
	 * loader or package.
	 */
	public static Builder valueDistribution(ISnapshot snapshot, IResultTable base, Target target)
		...
		try {
			Builder builder =
					new Builder(snapshot, base, target.getCalculatorClass().getDeclaredConstructor().newInstance());
	...
	/**
	 * A convenience methods to aggregate a table by one of the well-known targets and create the sum for all other
	 * columns.
	 */
	...
	public static final class Builder {
		...
		public Builder column(String label, Column baseColumn, int baseColumnIndex, Quantize.Function.Factory qff) {
			Column col = new Column(label,
					baseColumn.getType(),
					baseColumn.getAlign(),
					baseColumn.getSortDirection(),
					baseColumn.getFormatter(),
					baseColumn.getComparator()).decorator(baseColumn.getDecorator());
	...
	static class OneColumnFlatResult implements IResultTree, IIconProvider {
		...
		public OneColumnFlatResult(IResultTable table,
				List<Column> groupedColumns,
				List<GroupedRowImpl> elements,
				URL icon) {
			this.table = table;
	...
	static class ByClassloader extends Calculator {
		public IResult result(Map<Object, GroupedRowImpl> result) {
			return new OneColumnFlatResult(quantize.table,
					quantize.columns,
					new ArrayList<GroupedRowImpl>(result.values()),
	...
	static class ByPackageResult implements IResultTree, IIconProvider {
		...
		public ByPackageResult(Package root,
				IResultTable table,
				List<Column> groupedColumns,
				Map<Object, GroupedRowImpl> elements) {
			this.root = root;
	...
	/* package */static class BySingleColumn extends Calculator {
		...
		public IResult result(Map<Object, GroupedRowImpl> map) {
			return new OneColumnFlatResult(quantize.table,
					quantize.columns,
					new ArrayList<GroupedRowImpl>(map.values()),
					null);
		}
	...
	/* package */static class ByMultipleColumns extends Calculator {
		...
		public IResult result(Map<Object, GroupedRowImpl> map) {
			return new MultipleColumnsFlatResult(quantize.table,
					quantize.columns,
					new ArrayList<GroupedRowImpl>(map.values()));
		}
