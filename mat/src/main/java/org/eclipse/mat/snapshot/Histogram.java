...
/**
 * Class histogram - heap objects aggregated by their class. It holds the number and consumed memory of the objects
 * aggregated per class and aggregated per class loader.
 */
public class Histogram extends HistogramRecord implements IResultTable, IIconProvider {
	 ...
	 *
	 * @param label
	 *            the name of the whole histogram
	 * @param classHistogramRecords
	 *            the classes
	 * @param classLoaderHistogramRecords
	 *            the class loaders
	 * @param numberOfObjects
	 *            the total number of objects
	 * @param usedHeapSize
	 *            the shallow size of all the objects
	...
	/**
	 * Get collection of all the classes for all the objects which were found in the set of objects on which the class
	 * histogram was computed.
	 *
	 * @return collection of all the classes for all the objects which were found in the set of objects on which the
	...
	/**
	 * Get collection of all the class loaders for all the classes for all the objects which were found in the set of
	 * objects on which the class histogram was computed.
	 *
	 * @return collection of all the class loaders for all the classes for all the objects which were found in the set
	 *         of objects on which the class histogram was computed
	 */
	 ...
	 * @param baseline
	 *            baseline histogram
	 * @return difference histogram between this histogram compared to (minus) the given baseline histogram
	 */
	public Histogram diffWithBaseline(Histogram baseline) {
		...
		Map<String, Map<String, ClassHistogramRecord>> classLoaderDifferences =
				new HashMap<String, Map<String, ClassHistogramRecord>>();
		...
		for (ClassLoaderHistogramRecord classLoaderHistogramRecord : classLoaderHistogramRecords) {
			...
			for (ClassHistogramRecord classHistogramRecord : classLoaderHistogramRecord.getClassHistogramRecords()) {
				...
				if (classDifference == null) {
					classDifferences.put(className,
							classDifference = new ClassHistogramRecord(className, --classIdCurrent, 0, 0, 0));
				}
		...
		ArrayList<ClassLoaderHistogramRecord> classLoaderDiffRecordsMerged =
				new ArrayList<ClassLoaderHistogramRecord>();
		for (Map.Entry<String, Map<String, ClassHistogramRecord>> classDifferences : classLoaderDifferences
				.entrySet()) {
			ArrayList<ClassHistogramRecord> records =
					new ArrayList<ClassHistogramRecord>(classDifferences.getValue().values().size());
			...
			for (ClassHistogramRecord classDifference : classDifferences.getValue().values()) {
				...
				if (classDifferenceMerged == null) {
					classDiffRecordsMerged.put(classDifference.getLabel(),
							classDifferenceMerged =
									new ClassHistogramRecord(classDifference.getLabel(), --classIdCurrent, 0, 0, 0));
				}
			...
			classLoaderDiffRecordsMerged.add(new ClassLoaderHistogramRecord(classDifferences
					.getKey(), --classLoaderIdCurrent, records, numberOfObjects, usedHeapSize, 0));
		}
		Histogram histogram =
				new Histogram(MessageUtil.format(Messages.Histogram_Difference, getLabel(), baseline.getLabel()), //
	 ...
	 * <p>
	 * This method can be used to check what remains the same within two histograms, e.g. if you have two histograms it
	 * shows what hasn't changed, e.g. if you have two difference histograms it shows what remained the same change
	 * (increase or decrease; used in gradient memory leak analysis).
	 * <p>
	 * Note: Heap space is not taken into account in this analysis, only the number of objects, i.e. when the number of
	 * objects is the same, you will see this number of objects, otherwise or if there are no objects of a particular
	 * class you won't get a histogram record for it!
	 *
	 * @param another
	 *            another histogram
	 * @return intersection histogram of this histogram compared to (equals) the given another histogram
	 */
	public Histogram intersectWithAnother(Histogram another) {
		...
		for (ClassLoaderHistogramRecord classLoaderHistogramRecord : another.classLoaderHistogramRecords) {
			...
			for (ClassHistogramRecord classHistogramRecord : classLoaderHistogramRecord.getClassHistogramRecords()) {
				...
				if ((classDifference != null) && (classDifference.getNumberOfObjects() > 0)
					...
					classDifferences.put(className,
							new ClassHistogramRecord(className,
									--classIdCurrent,
									classDifference.getNumberOfObjects(),
									classDifference.getUsedHeapSize() + classHistogramRecord.getUsedHeapSize(),
									0));
				}
			}
		}
		...
		for (Map.Entry<String, Map<String, ClassHistogramRecord>> classDifferences : classLoaderDifferences
			...
			for (ClassHistogramRecord classDifference : classDifferences.getValue().values()) {
				if (!classDifference.getLabel().startsWith(prefix)) {
					...
					if (classDifferenceMerged == null) {
						classDiffRecordsMerged.put(classDifference.getLabel(),
								classDifferenceMerged = new ClassHistogramRecord(classDifference
										.getLabel(), --classIdCurrent, 0, 0, 0));
					}
					classDifferenceMerged.incNumberOfObjects(classDifference.getNumberOfObjects());
					classDifferenceMerged.incUsedHeapSize(classDifference.getUsedHeapSize());
					records.add(classDifference);
		...
		Histogram histogram =
				new Histogram(MessageUtil.format(Messages.Histogram_Intersection, getLabel(), another.getLabel()),
						new ArrayList<ClassHistogramRecord>(classDiffRecordsMerged.values()),
						classLoaderDiffRecordsMerged,
						numberOfObjectsOverall,
						usedHeapSizeOverall,
						0);
		histogram.showPlusMinus = true;
		return histogram;
	...
	public String toString() {
		...
		buf.append(MessageUtil.format(Messages.Histogram_Description, //
				label, //
	 ...
	 * @param comparator
	 *            comparator to be used for sorting the histogram records ( {@link HistogramRecord} provides some
	 *            default comparators)
	...
	public static String generateClassHistogramRecordTextReport(Histogram histogram,
		...
		return generateHistogramRecordTextReport(new ArrayList<HistogramRecord>(histogram.getClassHistogramRecords()),
				comparator,
				new String[] { Messages.Column_ClassName,
						Messages.Column_Objects,
						Messages.Column_Heap,
						Messages.Column_RetainedHeap });
	}
	...
	private static String generateHistogramRecordTextReport(List<HistogramRecord> records,
		...
		StringBuilder report = new StringBuilder((4 + records.size()) * (2 + labelLength
				+ 3
				+ numberOfObjectsLength
				+ 3
				+ usedHeapSizeLength
				+ 3
				+ retainedHeapSizeLength
				+ 2
				+ 2));
		appendStringAndFillUp(report,
				null,
				'-',
				2 + labelLength + 3 + numberOfObjectsLength + 3 + usedHeapSizeLength + 3 + retainedHeapSizeLength + 2);
		report.append("\r\n"); //$NON-NLS-1$
	 ...
	 *
	 * @param histogram
	 *            histogram you want a machine/human readable comma separated report for
	 * @param comparator
	 *            comparator to be used for sorting the histogram records ( {@link HistogramRecord} provides some
	 *            default comparators)
	 * @return machine/human readable comma separated report for the given histogram
	 */
	public static String generateClassHistogramRecordCsvReport(Histogram histogram,
			Comparator<HistogramRecord> comparator) {
		return generateHistogramRecordCsvReport(
				new ArrayList<ClassHistogramRecord>(histogram.getClassHistogramRecords()),
				comparator,
				new String[] { Messages.Column_ClassName,
						Messages.Column_Objects,
						Messages.Column_ShallowHeap,
						Messages.Column_RetainedHeap });
	}
	...
	public static String generateClassLoaderHistogramRecordCsvReport(Histogram histogram,
			Comparator<HistogramRecord> comparator) {
		return generateClassloaderHistogramCsvReport(
				new ArrayList<ClassLoaderHistogramRecord>(histogram.getClassLoaderHistogramRecords()),
				comparator,
				new String[] { Messages.Column_ClassLoaderName,
						Messages.Column_ClassName,
						Messages.Column_Objects,
						Messages.Column_ShallowHeap,
						Messages.Column_RetainedHeap });
	}
	...
	private static void appendPreFillAndString(StringBuilder report,
			String string,
			char character,
			int completeLength) {
		if (string != null) {
	...
	public Column[] getColumns() {
		...
		if (showPlusMinus) {
			...
			DecimalFormat detailedFormatter = new DecimalFormat("+" + BytesFormat.DETAILED_DECIMAL_FORMAT //$NON-NLS-1$
					+ ";-" //$NON-NLS-1$
					+ BytesFormat.DETAILED_DECIMAL_FORMAT
					+ ""); //$NON-NLS-1$
	...
	public final static class ClassLoaderTree implements IResultTree, IIconProvider {
		...
		public List<?> getChildren(Object parent) {
			return new ArrayList<ClassHistogramRecord>(
					((ClassLoaderHistogramRecord) parent).getClassHistogramRecords());
		}
				...
				return new IContextObjectSet() {
					...
					public String getOQL() {
						if (histogram.isDefaultHistogram)
							return OQL.instancesByClassLoaderId(record.getClassLoaderId());
						else
							return null;
					}
				};
	...
	public static final class PackageTree implements IResultTree, IIconProvider {
		...
		public Column[] getColumns() {
			return new Column[] {
					new Column(Messages.Histogram_Column_PackagePerClass, String.class)
							.comparing(HistogramRecord.COMPARATOR_FOR_LABEL), //
					new Column(Messages.Column_Objects, long.class) //
							.comparing(HistogramRecord.COMPARATOR_FOR_NUMBEROFOBJECTS), //
					new Column(Messages.Column_ShallowHeap, Bytes.class) //
	...
	public static final class SuperclassTree implements IResultTree, IIconProvider {
		...
		private void buildTree(Histogram histogram, ISnapshot snapshot) {
			...
			for (ClassHistogramRecord record : histogram.getClassHistogramRecords()) {
				...
				for (int i = l1.size() - 1; i >= 0; --i) {
					...
					if (child == null)
						current.subClasses.put(superclass.getObjectId(),
		...
		public Column[] getColumns() {
			return new Column[] {
					new Column(Messages.Histogram_Column_SuperclassPerClass, String.class)
							.comparing(HistogramRecord.COMPARATOR_FOR_LABEL), //
					new Column(Messages.Column_Objects, long.class) //
							.comparing(HistogramRecord.COMPARATOR_FOR_NUMBEROFOBJECTS), //
					new Column(Messages.Column_ShallowHeap, Bytes.class) //
