...
/**
 * This class holds all basic data for a histogram record. Other classes inherit from it. It offers comparators to sort
 * histogram records by their basic data. This pattern should be implemented by the classes inheriting from it as well.
 */
public class HistogramRecord implements Serializable {
	...
	public static final Comparator<HistogramRecord> COMPARATOR_FOR_RETAINEDHEAPSIZE =
			new Comparator<HistogramRecord>() {
				public int compare(HistogramRecord o1, HistogramRecord o2) {
	 ...
	 * <p>
	 * Retained bytes means how much memory would be garbage collected if the references to the objects this histogram
	 * record stands for would be lost and the objects garbage collected.
	 *
	 * @return number of retained bytes in heap area this histogram record stands for
	 */
	...
	/**
	 * Convenience method reversing the order of the given comparator. Be aware that each time you call this method a
	 * new comparator is returned and that this comparator delegates the call to the given comparator, but just switches
	 * the parameters. You can get the same result with the given comparator, by iterating from the last to the first
	 * element instead. From performance perspective using e.g. {@link java.util.List#get(int)} is anyhow faster
	 * compared to an {@link java.util.Iterator}.
	 *
	 * @param comparator
