...
/**
 * Instances of this class are descriptors of locally running Java processes. Arguments can be injected into the query
 * using public fields marked with the {@link Argument} annotation. Typical arguments to be supplied by the user of the
 * heap dump provider include
 * <ul>
 * <li>boolean flags</li>
 * <li>int parm</li>
...
public class VmInfo {
	 ...
	 * @param description
	 *            a free text description of the process, usually the process name
	 * @param heapDumpEnabled
	 *            a boolean value indicating if a heap dump from the process can be acquired
	 * @param proposedFileName
	 *            a proposal for the file name, under which the heap dump can be saved. %pid% can be used as a
	 *            placeholder for the PID. Example: java_pid%pid%.hprof
	...
	/**
	 * Returns a proposed file name under which the heap dump should be saved, e.g. java_pid%pid%.hprof for HPROF files
	 * or a file name template subject to substitution using {@link org.eclipse.mat.util.MessageUtil#format} with three
	 * parameters, {@link java.util.Date} date, int pid, int index For example:
	 * mydumpname.{0,date,yyyyMMdd.HHmmss}.{1,number,0}.{2,number,0000}.dmp
	 *
