...
public final class OQL {
	...
	/**
	 * Create a OQL union statement and append it to the query. Possibly optimize a common prefix. select s.a,s.b,s.c
	 * from 1,173 s select s.a,s.b,s.c from 123 s combine to select s.a,s.b,s.c from 1,173,123 s
	 *
	 * Also split off UNION clauses to see if the new clause can be merged into an existing UNION clause.
	 */
	...
	private static boolean union(StringBuilder query, int start, int end, String other) {
		...
		if (id1.equals(id2)) {
			...
			if (num1.length() > 0 && num2.length() > 0
					&& query.subSequence(start, s1).equals(other.subSequence(0, s2))) {
				int j = 0;
				while (isSpace(num2, j))
					++j;
	...
	private static final String OQL_classesByClassLoaderId = "SELECT * FROM java.lang.Class c WHERE c implements " //$NON-NLS-1$
			+ IClass.class.getName()
			+ " and c.@classLoaderId = {0,number,0}"; //$NON-NLS-1$
