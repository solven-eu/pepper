 ...
 * <p>
 * A snapshot basically has an {@link SnapshotInfo} object describing it and various methods operating on it. Be aware
 * that such a snapshot potentially holds quite a lot of memory, so try to keep not too many open at the same time.
 * <p>
 * The basic "handle" you will encounter in the API is an <code>int</code>. It is the id of an object and is always of
 * the size of four bytes (no matter if 32 or 64 bit). Operations are either performed on <code>int</code> or
 * <code>int[]</code> (addresses are stored in longs, but they should be used only for display purposes). In order to
 * simplify working with them and for performance reasons compared to <code>Collection&lt;Integer&gt;</code> some helper
 * classes are provided and you are strongly advised to use them: {@link org.eclipse.mat.collect.BitField} (for huge
 * fixed bit fields), {@link org.eclipse.mat.collect.ArrayIntBig} and {@link org.eclipse.mat.collect.ArrayLongBig} (for
 * huge growing arrays), {@link org.eclipse.mat.collect.ArrayIntCompressed} and
...
public interface ISnapshot {
	 ...
	 * Get all GC roots.
	 * <p>
	 * A GC root is an object which doesn't need to be referenced to remain in the heap, e.g. all objects created in the
	 * thread stack frame.
	 * <p>
	 * Performance: Fast - in memory.
	 * 
	 ...
	 * Get all classes.
	 * <p>
	 * The returned class is a snapshot representation of a Java class. It offers means to get to its name, class
	 ...
	 * <p>
	 * This method returns you for one full class name all classes with this name. Usually you will get only one class
	 * back, but in the case of multiple class loaders which have loaded a class with the same name twice (or even the
	 * same class) you will get multiple snapshot class representations back as only the combination of class and class
	 ...
	 *            flag indicating whether or not to include also classes derived from matching classes (the name isn't
	 ...
	 * <p>
	 * This histogram lists all classes and class loaders and how many objects of the classes exist in the snapshot
	 ...
	 * <p>
	 * This histogram lists all classes and class loaders and how many objects of the classes exist among the given
	 ...
	 * <p>
	 * Hint: This method is handy if you want to learn which classes reference a class. Therefore you would call this
	 * method with all objects of your class of interest and get a histogram out of the result.
	 * <p>
	 ...
	 * <p>
	 * Hint: This method is handy if you want to learn which classes are referenced by a class. Therefore you would call
	 * this method with all objects of your class of interest and get a histogram out of the result. This is the inverse
	 ...
	 * <p>
	 * Hint: This method is handy if you want to learn which objects are responsible for the given object to remain in
	 * memory. Since the snapshot implementation artificially creates references from the object to its class and from
	 * the class to its class loader you can even see why a class or class loaders remains in memory, i.e. which other
	 ...
	 * @param excludeMap
	 *            a map specifying paths through which objects have to be avoided and not reported. Each entry in the
	 *            map has the IClass as a key, and a Set&lt;String&gt; set, specifying which fields exactly from this
	 *            key class have to be avoided. If for a key IClass the value (Set&lt;String&gt;) null is specified,
	 ...
	 * <p>
	 * The retained set includes the given objects and all objects which are lifetime-dependent on them, i.e. which
	 * would be garbage collected if the references to the given objects would be lost and the objects garbage
	 * collected.
	 * <p>
	 ...
	 * <p>
	 * The retained set includes the objects referenced by the fields on the given objects and all objects which are
	 * lifetime-dependent on them, i.e. which would be garbage collected if the references at the given fields at the
	 ...
	 * <p>
	 * The minimum retained set includes the given objects and the union of the retained sets for each of the given
	 * objects (see getRetainedSet() for an explanation of a retained set). The union of the retained sets of the single
	 * objects is potentially smaller than the retained set of all of them, because some objects which are shared
	 * between two of the given objects may not appear in the retained set of any of the single objects, but will appear
	 * in the retained set if we take them as a whole. Because of it's faster performance the method is suitable to
	 * "mine" for a potentially big retained set (e.g. execute this method for all class loaders and see for potentially
	 ...
	 * class loaders
	 * <p>
	 * If an exclude pattern is provided, instead of returning the immediate dominator right away, its class name will
	 ...
	 *            An exclude pattern. Domminators whose class name matches the pattern will be omitted and their
	 *            dominator will be taken
	 * @param progressListener
	 *            progress listener informing about the current state of execution
	 * @return DominatorsSummary the returned DominatorSummary contains the summary of the dominators grouped by classes
	 *         or class loaders
	 * @throws SnapshotException
	 */
	 ...
	 * otherwise, a GCRootInfo[]. An object can be a GC root for more than one reason and the returned array will
	 * contain one instance of GCRootInfo for each of the reasons (e.g. one GCRootInfo for every thread where an object
	 ...
	 * this method now returns a long
	 * <p>
	 ...
	 * <p>
	 * The retained heap size is the memory which would be freed if all references to the given object would be
	 * released. It is extracted from the dominator tree and isn't available if the dominator tree isn't available
	 * itself.
	 * <p>
	 * Performance: Relatively fast - single index operation.
	 * 
	...
	/**
	 * Map object id (snapshot internal identity assigned during parsing) to object address (memory address where the
	 * object was stored).
	 * <p>
	 * Performance: Fast - in memory.
	 * 
