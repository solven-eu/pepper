...
/**
 * An abstract class representing a map extracted from the heap. It provides convenience methods for querying certain
 * properties of the collection (e.g. size) and for extracting the elements of the collection
 *
 * @since 1.5
 */
public class ExtractedMap extends AbstractExtractedCollection<Map.Entry<IObject, IObject>, IMapExtractor> {
	 ...
	 *
	 * @return the collision ratio as a {@link Double} between 0.0 and 1.0 inclusive, or possibly null
	...
	/**
	 * Gets an object from the Map, searching by the key. The keys are matched by IDENTITY, so cannot be used to compare
	 * for example Strings
	 *
	 * @param key
