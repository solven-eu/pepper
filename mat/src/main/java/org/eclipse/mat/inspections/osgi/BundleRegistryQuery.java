...
@Subjects({ "org.eclipse.osgi.framework.internal.core.BundleRepository",
		...
		public List<?> getChildren(Object parent) {
			if (parent instanceof BundleDescriptor) {
				...
				if (bundle.getExtentionPoints() != null && !bundle.getExtentionPoints().isEmpty())
					children.add(
							new Folder(bundle, Messages.BundleRegistryQuery_ExtensionPoints, Type.EXTENSION_POINTS));
				...
				if (bundle.getRegisteredServices() != null && !bundle.getRegisteredServices().isEmpty())
					children.add(new Folder(bundle,
							Messages.BundleRegistryQuery_RegisteredServices,
				...
				if (bundle instanceof BundleFragment && ((BundleFragment) bundle).getHost() != null)
					children.add(new Folder(bundle,
							MessageUtil.format(Messages.BundleRegistryQuery_HostedBy,
									((BundleFragment) bundle).getHost().getBundleName()),
							Type.HOST));
				...
				for (Extension extension : extensions) {
					String bundleName =
							(extension.getContributedBy() != null) ? extension.getContributedBy().getBundleName() : ""; //$NON-NLS-1$
					children.add(new ExtensionFolder(extension,
							MessageUtil.format(Messages.BundleRegistryQuery_ContributedBy, bundleName),
							Type.CONTRIBUTED_BY));
				}
				return children;
				...
				if (service.getProperties() != null)
					children.add(
							new PropertiesFolder(service, Messages.BundleRegistryQuery_Properties, Type.PROPERTIES));
				if (service.getBundlesUsing() != null && !service.getBundlesUsing().isEmpty())
					children.add(new PropertiesFolder(service,
							Messages.BundleRegistryQuery_BundlesUsing,
							Type.BUNDLES_USING));
				return children;
			}
		...
		public List<?> getChildren(Object parent) {
			if (parent instanceof Service) {
				...
				if (service.getProperties() != null)
					children.add(
							new PropertiesFolder(service, Messages.BundleRegistryQuery_Properties, Type.PROPERTIES));
				children.add(new DescriptorFolder(service.getBundleDescriptor(),
						MessageUtil.format(Messages.BundleRegistryQuery_RegisteredBy,
								service.getBundleDescriptor().getBundleName()),
						Type.BUNDLE));
				if (service.getBundlesUsing() != null && !service.getBundlesUsing().isEmpty())
					children.add(new PropertiesFolder(service,
							Messages.BundleRegistryQuery_BundlesUsing,
		...
		public List<?> getChildren(Object parent) {
			if (parent instanceof ExtensionPoint) {
				...
				if (point.getContributedBy() != null)
					children.add(new DescriptorFolder(point.getContributedBy(),
							MessageUtil.format(Messages.BundleRegistryQuery_RegisteredBy,
									point.getContributedBy().getBundleName()),
							Type.BUNDLE));
				for (Extension extension : extensions) {
					if (extension.getContributedBy() != null)
						children.add(new ExtensionFolder(extension,
								MessageUtil.format(Messages.BundleRegistryQuery_ContributedBy,
										extension.getContributedBy().getBundleName()),
								Type.CONTRIBUTED_BY));
				}
