...
/**
 * Extract information about objects referenced by java.lang.ref.Reference, e.g. weak and soft references, and Finalizer
 * which are also strongly held by the reference, so causing a possible leak.
 */
...
public class ReferenceLeakQuery implements IQuery {
	...
	public IResult execute(IProgressListener listener) throws Exception {
		...
		SimpleMonitor sm =
				new SimpleMonitor(Messages.ReferenceLeakQuery_ComputingReferentLeaks, listener, new int[] { 400, 100 });
			...
			for (int ii = 0; ii < objs.length; ii++, iobjs++) {
				...
				if (o instanceof IInstance) {
					...
					if (ref != null) {
						...
						try {
							...
							if (++errs <= 20)
								listener.sendUserMessage(IProgressListener.Severity.WARNING,
										MessageUtil.format(Messages.ReferenceLeakQuery_ErrorReadingReference,
												obj,
												referent_attribute,
												ref),
										e);
						...
						int count = 0;
						lx: for (int p1[] = path.getNextShortestPath(); p1 != null && count < maxpaths; p1 =
								path.getNextShortestPath(), ++count) {
							for (int ip = 0; ip < p1.length; ++ip) {
								...
								if (p == obj.getObjectId()) {
									...
									if (ai.size() <= maxresults) {
										...
										String title = MessageFormat.format(Messages.ReferenceLeakQuery_TwoPaths,
												ref.getObject().getDisplayName(),
												o.getDisplayName());
		...
		if (ai.size() == 1) {
			...
			IMultiplePathsFromGCRootsComputer computer =
					snapshot.getMultiplePathsFromGCRoots(ai.toArray(), allExcludeMap);
		...
		if (ai.size() > 1) {
			...
			/*
			 * Using the paths from getPathsFromGCRoots doesn't always match getMultiplePathsFromGCRoots so redo.
			 */
