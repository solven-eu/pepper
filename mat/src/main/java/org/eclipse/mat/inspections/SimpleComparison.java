...
public class SimpleComparison implements IQuery {
	...
	public String defaultoptions =
			"-mask \"\\s@ 0x[0-9a-f]+|^(\\[[0-9]+\\], )*\\[[0-9]+\\]$|(?<=\\p{javaJavaIdentifierPart}\\[)\\d+(?=\\])\" -x java.util.HashMap$Node:key java.util.Hashtable$Entry:key java.util.WeakHashMap$Entry:referent java.util.concurrent.ConcurrentHashMap$Node:key"; //$NON-NLS-1$
	...
	private IStructuredResult callQuery(IProgressListener listener, ISnapshot snapshot) throws Exception {
		if (retained != null) {
			...
			rb1.addDefaultContextDerivedColumn(retained == Retained.PRECISE ? RetainedSizeDerivedData.PRECISE
					: RetainedSizeDerivedData.APPROXIMATE);
			return rb1.build();
		} else {
			return (IStructuredResult) SnapshotQuery.parse(query, snapshot).execute(listener);
		}
	}
}
