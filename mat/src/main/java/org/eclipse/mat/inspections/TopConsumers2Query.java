...
public class TopConsumers2Query implements IQuery {
	...
	private static Column getColRetainedHeap() {
		...
		return new Column(Messages.TopConsumers2Query_Column_RetainedHeapPercent, double.class).formatting(nf)
				.noTotals();
	}
	...
	private void addTopLevelDominatorClasses(SectionSpec composite, Histogram histogram, IProgressListener listener) {
		...
		for (ClassHistogramRecord record : records) {
			...
			suspects.add(record);
			pie.addSlice(record.getClassId(),
					record.getLabel(), //
		...
		if (suspects.isEmpty()) {
			...
			if (spec.getCommand() == null && objects == null) {
				...
				sb.insert(0,
						"show_dominator_tree -groupby BY_CLASS SELECT OBJECTS o FROM OBJECTS (dominators(-1)) o where classof(o) in (SELECT * FROM OBJECTS "); //$NON-NLS-1$
				sb.append(')');
				spec.setCommand(sb.toString());
			}
			spec.set(Params.Html.COLLAPSED, Boolean.TRUE.toString());
	...
	private void addTopLevelDominatorClassloader(SectionSpec composite,
			Histogram histogram,
			IProgressListener listener) {
		ClassLoaderHistogramRecord[] records =
				histogram.getClassLoaderHistogramRecords().toArray(new ClassLoaderHistogramRecord[0]);
		Arrays.sort(records, Histogram.reverseComparator(Histogram.COMPARATOR_FOR_RETAINEDHEAPSIZE));
		...
		for (ClassLoaderHistogramRecord record : records) {
			if (record.getRetainedHeapSize() <= threshold)
				break;
			suspects.add(record);
			pie.addSlice(record.getClassLoaderId(),
					record.getLabel(), //
					record.getUsedHeapSize(),
					record.getRetainedHeapSize());
		}
		...
		if (suspects.isEmpty()) {
			...
			ListResult result = new ListResult(ClassLoaderHistogramRecord.class,
					suspects, //
					"label", //$NON-NLS-1$
					"numberOfObjects", //$NON-NLS-1$
					"usedHeapSize", //$NON-NLS-1$
			...
			try {
				...
				if (spec.getCommand() == null && objects == null) {
					...
					sb.setLength(0);
					sb.append(
							"show_dominator_tree -groupby BY_CLASSLOADER SELECT OBJECTS o FROM OBJECTS (dominators(-1)) o WHERE classof(o).@classLoaderId in "); //$NON-NLS-1$
					sb.append(ids);
					sb.append(
							" and o implements org.eclipse.mat.snapshot.model.IClass = false and o implements org.eclipse.mat.snapshot.model.IClassLoader = false"); //$NON-NLS-1$
					sb.append(
							" or o implements org.eclipse.mat.snapshot.model.IClass and $ {snapshot}.getObject(o).@classLoaderId in "); //$NON-NLS-1$
					sb.append(ids);
	...
	private void addPackageTree(SectionSpec spec, IProgressListener listener) throws SnapshotException {
		...
		QuerySpec querySpec =
				new QuerySpec(Messages.TopConsumers2Query_BiggestPackages, new PackageTreeResult(root, totalHeap));
	...
	static private class ClassHistogramRecordWithObjIds extends ClassHistogramRecord {
		...
		public ClassHistogramRecordWithObjIds(String label,
				int classId,
	...
	private Histogram getDominatedHistogramWithRetainedSizes(IProgressListener listener) throws SnapshotException {
		...
		HashMapIntObject<ClassHistogramRecordWithObjIds> id2class =
				new HashMapIntObject<ClassHistogramRecordWithObjIds>();
		...
		for (int ii = 0; ii < topDominators.length; ii++) {
			...
			if (loaderRecord == null) {
				...
				loaderRecord = new ClassLoaderHistogramRecord(name,
						loader.getObjectId(),
						new ArrayList<ClassHistogramRecord>(),
						0,
						0,
						0);
