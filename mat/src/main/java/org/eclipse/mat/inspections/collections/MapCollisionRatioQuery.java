...
public class MapCollisionRatioQuery implements IQuery {
	...
	public IResult execute(IProgressListener listener) throws Exception {
		...
		Quantize.Builder builder =
				Quantize.linearFrequencyDistribution(Messages.MapCollisionRatioQuery_Column_CollisionRatio,
						0,
						1,
						(double) 1 / (double) segments);
		...
		for (int[] objectIds : objects) {
			...
			for (int objectId : sortedObjs) {
				...
				if (counter++ % 1000 == 0 && !obj.getClazz().equals(type)) {
					type = obj.getClazz();
					listener.subTask(
							Messages.MapCollisionRatioQuery_CalculatingCollisionRatios + "\n" + type.getName()); //$NON-NLS-1$
				}
				try {
					...
					if (coll != null) {
						/*
						 * @FIXME - shouldn't really count maps without a collision ratio but current tests presume
						 * TreeSet/TreeMap have one.
						 */
