...
public class ComponentReportQuery implements IQuery {
	...
	private static final String UNFINALIZED_REFERENCE =
			"<" + GCRootInfo.getTypeAsString(GCRootInfo.Type.UNFINALIZED) + ">"; //$NON-NLS-1$ //$NON-NLS-2$

	public IResult execute(IProgressListener listener) throws Exception {
		SectionSpec componentReport =
				new SectionSpec(MessageUtil.format(Messages.ComponentReportQuery_ComponentReport, objects.getLabel()));
	...
	/**
	 * Purpose: absorb progress reports from sub-queries to report a smoother overall progress
	 */
	...
	private int[] calculateRetainedSize(Ticks ticks) throws SnapshotException {
		...
		if (excludes.isEmpty()) {
			...
			if (useoql) {
				/*
				 * See if the ordinary retained set as used by OQL returns the same results Do this first so we can
				 * discard the result and just check the length Consider o is component roots, z = other roots,
				 * SR=SoftReference o->SR->A o->B z->SR->B If SoftRefs followed, retained set = SR,A if SoftRefs not
				 * followed, retained set = SR,B so same number of refs doesn't prove equality. Order doesn't matter
				 * though.
				 */
	...
	private void addOverview(SectionSpec componentReport,
			long totalSize,
			int[] retained,
			Histogram histogram,
	...
	private void addOverviewNumbers(int[] retained, Histogram histogram, SectionSpec overview, long totalSize) {
		...
		buf.append(Messages.ComponentReportQuery_Size)
				.append(" <strong>") //$NON-NLS-1$
				.append(Units.Storage.of(totalSize).format(totalSize))
				.append("</strong> "); //$NON-NLS-1$
		buf.append(Messages.ComponentReportQuery_Classes)
				.append(" <strong>") //$NON-NLS-1$
				.append(Units.Plain.of(noOfClasses).format(noOfClasses))
				.append("</strong> "); //$NON-NLS-1$
		buf.append(Messages.ComponentReportQuery_Objects)
				.append(" <strong>") //$NON-NLS-1$
				.append(Units.Plain.of(noOfObjects).format(noOfObjects))
				.append("</strong> "); //$NON-NLS-1$
		buf.append(Messages.ComponentReportQuery_ClassLoader)
				.append(" <strong>") //$NON-NLS-1$
				.append(Units.Plain.of(noOfClassLoaders).format(noOfClassLoaders))
				.append("</strong>"); //$NON-NLS-1$
	...
	private void addDuplicateStrings(SectionSpec componentReport, long totalSize, Histogram histogram, Ticks listener)
			throws Exception {
		...
		for (ClassHistogramRecord record : sorted(histogram.getClassHistogramRecords())) {
			...
			for (int rowId = 0; rowId < table.getRowCount() && rowId < 5; rowId++) {
				...
				if (!foundStrings) {
					foundStrings = true;
					comment.append(
							HTMLUtils.escapeText(MessageUtil.format(Messages.ComponentReportQuery_Msg_FoundOccurrences,
									table.getRowCount(),
									totals.getLabel(2),
									record.getLabel(),
									dupcount)));
					comment.append("<p>") //$NON-NLS-1$
							.append(Messages.ComponentReportQuery_TopElementsInclude)
							.append("</p><ul title=\"") //$NON-NLS-1$
	...
	private void addCommand(QuerySpec spec, String command, int suspects[], IClass clazz) {
		if (suspects.length > 0 && suspects.length <= 30) {
			...
			String oql = "select * from objects (" + oqlretained //$NON-NLS-1$
					+ ") m where m.@clazz.@objectAddress = " //$NON-NLS-1$
				...
				for (int rowId = 0; rowId < count && rowId < 10; rowId++) {
					...
					if (collectionSize == 0) {
						...
						if (size > threshold) {
							...
							if (comment.length() == 0) {
								comment.append(
										HTMLUtils.escapeText(Messages.ComponentReportQuery_DetectedEmptyCollections))
							...
							comment.append(MessageUtil.format(Messages.ComponentReportQuery_Msg_InstancesRetainBytes,
									...
									HTMLUtils.escapeText(retainedSize)));
							comment.append("</li>"); //$NON-NLS-1$
							warn = true;
						}
				...
				for (int rowId = 0; rowId < count && rowId < 10; rowId++) {
					...
					if (arraySize <= 0) {
						...
						if (size > threshold) {
							...
							if (comment.length() == 0) {
								comment.append(
										HTMLUtils.escapeText(Messages.ComponentReportQuery_DetectedZeroLengthArray))
										.append("<ul title=\"") //$NON-NLS-1$
										.append(escapeHTMLAttribute(
			...
			if (prim) {
				...
				for (int rowId = 0; rowId < count; rowId++) {
					...
					if (size > threshold) {
						...
						String retainedSize = refinedTable.getFormattedColumnValue(row, 3);
						String cn = clazz.getName();
						String an = cn.substring(0,
								cn.length() - 1) + length + cn.substring(cn.length() - 1) + " {" + value + "}"; //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void primitiveArrayComment(StringBuilder comment, int numberOfObjects, String retainedSize, String an) {
		if (comment.length() == 0) {
			comment.append(HTMLUtils.escapeText(Messages.ComponentReportQuery_DetectedTheFollowingPrimitiveArray))
					.append("<ul title=\"") //$NON-NLS-1$
					.append(escapeHTMLAttribute(Messages.ComponentReportQuery_DetectedTheFollowingPrimitiveArray))
					.append("\">"); //$NON-NLS-1$
		}
	...
	private void addCollectionFillRatios(SectionSpec componentReport,
			long totalSize,
			Histogram histogram,
			Ticks listener) throws Exception {
		// Works, but very slow for some dump types, so disable for them.
		...
		for (ClassHistogramRecord record : sorted(histogram.getClassHistogramRecords())) {
			...
			ICollectionExtractor extractor = CollectionExtractionUtils.findCollectionExtractor(clazz.getName());
			if (extractor != null && extractor.hasSize()
					&& (extractor.hasFillRatio() || extractor.hasCapacity() || extractor.hasExtractableContents())) {
				// run the query: collections by size
						...
						if (size > threshold) {
							...
							if (comment.length() == 0) {
								comment.append(HTMLUtils
										.escapeText(Messages.ComponentReportQuery_Msg_DetectedCollectionFillRatios))
										.append("<ul title=\"") //$NON-NLS-1$
										.append(escapeHTMLAttribute(
		...
		QuerySpec commentSpec =
				new QuerySpec(Messages.ComponentReportQuery_Comment, new TextResult(comment.toString(), true));
	...
	private void addArrayFillRatios(SectionSpec componentReport, long totalSize, Histogram histogram, Ticks listener)
			throws Exception {
		// Works, but very slow for some dump types, so disable for them.
		if (!aggressive)
			InspectionAssert.heapFormatIsNot(snapshot, "DTFJ-PHD"); //$NON-NLS-1$
		...
		for (ClassHistogramRecord record : sorted(histogram.getClassHistogramRecords())) {
			...
			if (clazz.isArrayType()) {
				...
				for (int rowId = 0; rowId < count; rowId++) {
					...
					/*
					 * Empty arrays, non-zero length arrays should be counted here. Collections are not because they
					 * have a query for size 0 collections.
					 */
					if (fillRatio >= 0d && fillRatio < 0.21d) {
						int retainedCol = 4;
						long size = Math.abs((Long) refinedTable.getColumnValue(row, retainedCol));
	...
	private void addHashMapsCollisionRatios(SectionSpec componentReport,
			long totalSize,
			Histogram histogram,
			Ticks listener) throws Exception {
		// Works, but very slow for some dump types, so disable for them.
		...
		for (ClassHistogramRecord record : sorted(histogram.getClassHistogramRecords())) {
			...
			if (extractor != null && extractor instanceof IMapExtractor
					&& ((IMapExtractor) extractor).hasCollisionRatio()) {
				// run the query: collections by size
				int[] objectIds = record.getObjectIds();
				...
				for (int rowId = 0; rowId < count; rowId++) {
					...
					if (collisionRato > 0.80d) {
						...
						if (comment.length() == 0) {
							comment.append(
									HTMLUtils.escapeText(Messages.ComponentReportQuery_Msg_DetectedCollisionRatios))
									.append("<ul title=\"") //$NON-NLS-1$
									.append(escapeHTMLAttribute(
	...
	private static class SoftReferenceMessages extends ReferenceMessages {
		{
			...
			ReferenceStatQuery_Label_StronglyRetainedReferents =
					Messages.SoftReferenceStatQuery_Label_StronglyRetainedReferents;
	...
	private static class WeakReferenceMessages extends ReferenceMessages {
		{
			...
			ReferenceStatQuery_Label_StronglyRetainedReferents =
					Messages.WeakReferenceStatQuery_Label_StronglyRetainedReferents;
	...
	private void addReferenceStatistic(SectionSpec componentReport,
			Histogram histogram,
			Ticks ticks,
			String className,
		...
		Histogram softRefHistogram =
				new Histogram(messages.HistogramOfReferences, softRefs, null, numObjects, heapSize, 0);

		CompositeResult referents = ReferenceQuery.execute(instanceSet,
				referentSet,
				snapshot,
				messages.ReferenceStatQuery_Label_Referenced,
				messages.ReferenceStatQuery_Label_Retained,
				messages.ReferenceStatQuery_Label_StronglyRetainedReferents,
				"referent", //$NON-NLS-1$
				ticks);
		...
		if (numObjects >= 1) {
			comment.append("<strong>") //$NON-NLS-1$
					.append(MessageUtil.format(Messages.ComponentReportQuery_PossibleMemoryLeak))
					.append("</strong> "); //$NON-NLS-1$
		}
		...
		/*
		 * We have some candidates. Find the path from the suspect to the roots excluding weak referents and see if it
		 * goes through a weak reference which refers to this suspect. If so, it is a candidate problem. Record it, and
		 * the path and the path by class.
		 */
		if (numObjects >= 1) {
			// Number of paths to show per referent type
			int maxsuspectspertype = 1000;
			/*
			 * maximum paths to examine It is possible that the shortest path doesn't go through the actual reference
			 * but through others.
			 */
			int maxpaths = 10;
			double factor = 0.6;
			...
			for (ClassHistogramRecord cr : stronglyReachableReferents.getClassHistogramRecords()) {
				...
				if (result instanceof CompositeResult) {
					...
					if (!(entries.size() == 1 && entries.get(0).getResult() instanceof TextResult)) {
						overview.setStatus(ITestResult.Status.WARNING);
						commentSpec.set(Params.Html.IS_IMPORTANT, Boolean.TRUE.toString());
						QuerySpec child1 = new QuerySpec(
								MessageUtil.format(Messages.ComponentReportQuery_ExampleLeakDetails, cr.getLabel()),
								cr1);
						...
						QuerySpec child1 = new QuerySpec(
								MessageUtil.format(Messages.ComponentReportQuery_ReferentResult, cr.getLabel()),
								tr);
						child1.set(Params.Html.COLLAPSED, Boolean.TRUE.toString());
						overview.add(child1);
					}
	...
	private void addFinalizerStatistic(SectionSpec componentReport, int[] retained, Ticks ticks)
		...
		if (!foundSomeFinalizers) {
			...
			addEmptyResult(componentReport,
					Messages.ComponentReportQuery_FinalizerStatistics,
