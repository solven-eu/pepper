...
/**
 * Used to make it easier to track progress with an {@link IProgressListener} when going through an
 * {@link IHeapObjectArgument#iterator()}. Measuring the size in advance could be expensive if the argument is an OQL
 * query, so we can approximate the work to be done as each int[] is read.
 *
 * <pre>{@code
...
class HeapObjectsTracker {
	...
	/**
	 * Construct the tracker from a {@link IHeapObjectArgument} supplied to a query.
	 *
	 * @param objects
	 */
	public HeapObjectsTracker(IHeapObjectArgument objects) {
		...
		{
			/*
			 * This could be expensive to evaluate twice, so use the estimation for work done.
			 */
			totalWork = 1000000;
			actual = -1;
		} else {
	...
	/**
	 * Starts progress tracker for an array of ints from the iterator {@link IHeapObjectArgument#iterator()}
	 *
	 * @param objectIds
	 *            The object ids
	 * @param last
	 *            whether this is the last block or not
	 */
	...
	public int work() {
		++k;
		int work = (int) ((long) k * (totalWork - workPrev) / est) + workPrev;
	...
	/**
	 * Calculates the work done at the end of processing an array of ints from the iterator
	...
	public int endBlock() {
		int delta;
		if (k < n) {
			int work = (int) ((long) n * (totalWork - workPrev) / est) + workPrev;
			delta = work - workDone;
			workDone = work;
		} else {
			delta = 0;
