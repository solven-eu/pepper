...
public class AbstractFillRatioQuery {
	...
	protected void runQuantizer(IProgressListener listener,
			Quantize quantize,
			ICollectionExtractor specificExtractor,
			String specificClass,
			ISnapshot snapshot,
			Iterable<int[]> objects,
			String msg) throws SnapshotException {
		SnapshotInfo info = snapshot.getSnapshotInfo();
		final int refsize =
				info.getIdentifierSize() == 8 && Boolean.TRUE.equals((Boolean) info.getProperty("$useCompressedOops")) //$NON-NLS-1$
						? 4
		...
		for (int[] objectIds : objects) {
			...
			for (int objectId : sortedObjs) {
				...
				try {
					AbstractExtractedCollection<?, ?> coll =
							CollectionExtractionUtils.extractCollection(obj, specificClass, specificExtractor);
