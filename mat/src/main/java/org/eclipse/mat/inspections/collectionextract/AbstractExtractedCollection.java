...
/**
 * An abstract class representing a collection extracted from the heap. It provides convenience methods for querying
 * certain properties of the collection (e.g. size) and for extracting the elements of the collection
 *
 * @since 1.5
 */
public abstract class AbstractExtractedCollection<E, X extends ICollectionExtractor> implements Iterable<E>, IObject {
	...
	/**
	 * Returns the ICollectionExtractor used to extract the data about the collection
	 *
	 * @return
	 */
	...
	/**
	 * Get the size of the collection. This is the number of entries it holds.
	 *
	 ...
	 *
	 * @return true if the collection is empty, either {@link #size()} is zero or there are no entry ids.
	 */
	...
	public String toString() {
		return MessageUtil.format(Messages.AbstractExtractedCollection_ToString,
				getClass().getName(),
				getCollection().getDisplayName());
	}
}
