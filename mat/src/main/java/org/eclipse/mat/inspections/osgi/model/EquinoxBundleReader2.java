...
public class EquinoxBundleReader2 implements IBundleReader {
	...
	private Map<BundleDescriptor, List<ExtensionPoint>> extensionPointsByBundle =
			new HashMap<BundleDescriptor, List<ExtensionPoint>>();
	// bundle name -> Extensions
	private Map<BundleDescriptor, List<Extension>> extensionsByBundle =
			new HashMap<BundleDescriptor, List<Extension>>();
	// bundle name -> dependencies
	private Map<BundleDescriptor, List<BundleDescriptor>> bundleDependencies =
			new HashMap<BundleDescriptor, List<BundleDescriptor>>();
	// bundle name -> dependents
	private Map<BundleDescriptor, List<BundleDescriptor>> bundleDependents =
			new HashMap<BundleDescriptor, List<BundleDescriptor>>();
	...
	private List<BundleDescriptor> getBundleDescriptors(IProgressListener listener) throws SnapshotException {
		...
		Collection<IClass> classes =
				snapshot.getClassesByName("org.eclipse.osgi.internal.framework.EquinoxBundle", true); //$NON-NLS-1$
	...
	/**
	 * Find the capabilities for a bundle. Copes with <= 2021-03, using regular collection, or >= 2021-06 using
	 * org.eclipse.osgi.internal.container.NamespaceList
	 *
	 * @param rev
	...
	private List<Service> collectServiceInfo(IProgressListener listener) throws SnapshotException {
		...
		Collection<IClass> classes = snapshot.getClassesByName( // 3.5
				"org.eclipse.osgi.internal.serviceregistry.ServiceRegistry", //$NON-NLS-1$
				false);
		if (classes == null || classes.isEmpty())
			classes = snapshot.getClassesByName( // 3.4
					"org.eclipse.osgi.framework.internal.core.ServiceRegistryImpl", //$NON-NLS-1$
					false);
						...
						if (em != null && em.hasSize()) {
							...
							if (keysArray != null) {
								...
								if (keyAddresses != null) {
									keys = getServiceProperties(new String[keyAddresses.length],
											keyAddresses,
											listener);
								}
							}
							...
							if (valuesArray != null) {
								...
								if (valueAddresses != null) {
									values = getServiceProperties(new String[valueAddresses.length],
											valueAddresses,
											listener);
								}
							}
	...
	private void doUpdate(Service service,
	...
	private Bundle load(BundleDescriptor descriptor) throws SnapshotException {
		...
		if (descriptor.getType().equals(BundleDescriptor.Type.FRAGMENT)) {
			...
			return new BundleFragment(descriptor,
					location,
					host,
					dependencies,
					dependents,
	...
	private BundleDescriptor getFragmentHost(IObject bundleFragmentObject) throws SnapshotException {
				...
				if ("equinox.fragment".equals(((IObject) (o.resolveValue("namespace"))).getClassSpecificName())) //$NON-NLS-1$ //$NON-NLS-2$
				{
					...
					for (Entry<IObject, IObject> et : kvs) {
						if ("equinox.fragment".equals(et.getKey().getClassSpecificName())) //$NON-NLS-1$
						{
							...
							if (hostName != null) {
								for (BundleDescriptor bd : bundleDescriptors.values()) {
									if (bd.getBundleName().equals(hostName)
											|| bd.getBundleName().startsWith(hostName + " (")) //$NON-NLS-1$
									{
										return bd;
									}
								}
		...
		MATPlugin.log(IStatus.WARNING,
				MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_BundleNotFound,
		...
		if (classes != null) {
			...
			for (IClass clazz : classes) {
				...
				for (int i = 0; i < objs.length; i++) {
					...
					IObject providerBundleHostObject =
							(IObject) obj.resolveValue("hostingProvider.revisions.module.this$0");//$NON-NLS-1$
					...
					IObject requirerBundleHostObject =
							(IObject) obj.resolveValue("hostingRequirer.revisions.module.this$0");//$NON-NLS-1$
	...
	private List<ExtensionPoint> collectExtensionsInfo(IProgressListener listener) throws SnapshotException {
		...
		Collection<IClass> classes =
				snapshot.getClassesByName("org.eclipse.core.internal.registry.ExtensionRegistry", false); //$NON-NLS-1$
	...
	private void extractElements(IObject instance,
			Map<String, ExtensionPoint> extensionPoints,
			Map<Integer, Extension> extensions,
			Map<Integer, ConfigurationElement> configElements,
		...
		if (className.equals("org.eclipse.core.internal.registry.ExtensionPoint")) //$NON-NLS-1$
		{
			...
			if (extensionPoint != null && !extensionPoints.containsValue(extensionPoint))
				...
				listener.sendUserMessage(Severity.WARNING,
						MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_DuplicateExtensionPoint,
		...
		} else if (className.equals("org.eclipse.core.internal.registry.ConfigurationElement")) //$NON-NLS-1$
		{
			...
			if (configElement != null && !configElements.containsValue(configElement))
				...
				listener.sendUserMessage(Severity.WARNING,
						MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_DuplicateConfigurationElement,
		...
		} else if (className.equals("org.eclipse.core.internal.registry.Extension")) //$NON-NLS-1$
		{
			...
			if (extension != null && !extensions.containsValue(extension))
				...
				listener.sendUserMessage(Severity.WARNING,
						MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_DuplicateExtension,
								extension.getName(),
			...
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_UnknownElementType,
							Long.toHexString(instance.getObjectAddress()),
							instance.getClazz().getName()),
					null);
		}
	}
	...
	private ConfigurationElement extractConfigurationElementInfo(IObject instance, IProgressListener listener)
		...
		if (idField == null) {
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_ExpectedFieldParent,
							Long.toHexString(instance.getObjectAddress())),
					null);
			return null;
		}
		...
		if (objectIdField == null) {
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_ExpectedFieldObjectId,
							Long.toHexString(instance.getObjectAddress())),
					null);
			return null;
		}
		...
		if (contributorObject == null) {
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_ExpectedFieldContributorId,
							Long.toHexString(instance.getObjectAddress())),
					null);
			return null;
		}
		...
		/*
		 * in some heap dumps the the contributorID was a fully qualified name instead of a number. The following lines
		 * are an attempt to read the data as number
		 */
		...
		try {
			...
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_CannotFindContributorID,
							contributorIdString,
							Long.toHexString(instance.getObjectAddress())),
					e);
		}
		...
		if (propertiesObject == null) {
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_ExpectedFieldPropertiesAndValues,
							Long.toHexString(instance.getObjectAddress())),
					null);
			...
			return new ConfigurationElement(instance
					.getObjectId(), name, parentId, objectId, contributedBy, propertiesAndValues);
		} else {
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_ExpectedStringArray,
							Long.toHexString(instance.getObjectAddress())),
					null);
		}
	...
	private Extension extractExtensionInfo(IObject instance,
			Map<String, String> contributors,
			IProgressListener listener) throws SnapshotException {
	...
	private String[] getExtensionProperties(IObject instance, IProgressListener listener) throws SnapshotException {
		...
		if (extraInfoObject == null) {
			...
			listener.sendUserMessage(Severity.WARNING,
					MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_ExpectedFieldExtraInformation,
							Long.toHexString(instance.getObjectAddress())),
					null);
			return null;
		}
		...
		if (extraInfoObject.getClazz().isArrayType()) {
			...
			if (referentObject != null)
				listener.sendUserMessage(Severity.WARNING,
						MessageUtil.format(Messages.EquinoxBundleReader_ErrorMsg_SoftReferencesNotHandled,
								Long.toHexString(instance.getObjectAddress())),
						null);
			return null;
		}
	...
	private String[] getExtensionPointProperties(IObject instance) throws SnapshotException {
		IObjectArray nameArray =
				(IObjectArray) instance.resolveValue("registry.registryObjects.extensionPoints.keyTable");//$NON-NLS-1$
		IPrimitiveArray idArray =
				(IPrimitiveArray) instance.resolveValue("registry.registryObjects.extensionPoints.valueTable");//$NON-NLS-1$
	...
	private ExtensionPoint extractExtensionPointInfo(IObject instance,
			Map<String, String> contributors,
			IProgressListener listener) throws SnapshotException {
