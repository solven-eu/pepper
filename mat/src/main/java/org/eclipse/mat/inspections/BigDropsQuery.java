...
public class BigDropsQuery implements IQuery, IResultTree {
	...
	private void buildTree(long threshold, IProgressListener listener) throws SnapshotException {
		/*
		 * The algorithm implies that the children are sorted by their retained size descending. This is how the data is
		 * stored in the index
		 */

		StackEntry entry = new StackEntry(ROOT_ID,
				snapshot.getSnapshotInfo().getUsedHeapSize(),
				snapshot.getImmediateDominatedIds(ROOT_ID),
				0);
		Stack<StackEntry> stack = new Stack<StackEntry>();
		stack.push(entry);

		rootEntry = new BigDropEntry(ROOT_ID,
				ROOT_LABEL,
				snapshot.getSnapshotInfo().getUsedHeapSize(),
		...
		while (stack.size() > 0) {
			...
			if (entry.nextChild == 0) // first time we see the parent
			{
				if (entry.children.length == 0) // a leaf node
				{
					if (entry.parentSize > threshold) {
						...
						if (stack.size() > 1) // not a top-level
						{
							...
							{
								newBigDrop = new BigDropEntry(obj.getObjectId(),
										obj.getDisplayName(),
										obj.getRetainedHeapSize(),
										entry.children.length,
										ROOT_ID,
										ROOT_LABEL,
								...
								newBigDrop = new BigDropEntry(obj.getObjectId(),
										...
										obj.getRetainedHeapSize(),
										entry.children.length,
										dominatorObject.getObjectId(),
										dominatorObject.getDisplayName(),
										dominatorObject.getRetainedHeapSize());
							}
				...
				// there are some children -> check for big drop
				{
					...
					{
						stack.pop();
						stack.push(new StackEntry(entry.children[entry.nextChild],
								childRetainedSize,
								snapshot.getImmediateDominatedIds(entry.children[entry.nextChild]),
								0));
						continue;
					}
				}
	...
	/*
	 * returns the first dominator not matching the pattern or null (for the <ROOT>)
	 */
