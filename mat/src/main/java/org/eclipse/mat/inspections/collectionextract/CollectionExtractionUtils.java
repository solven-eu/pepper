...
/**
 * Utility class providing helpers simplifying the extraction of data from objects in the heap which represent
 * collections
 *
 * @since 1.5
 */
public class CollectionExtractionUtils {
	private static Map<ISnapshot, HashMapIntObject<ICollectionExtractor>> cache =
			new WeakHashMap<ISnapshot, HashMapIntObject<ICollectionExtractor>>();
	...
	public static ICollectionExtractor findCollectionExtractor(IObject collection) throws SnapshotException {
		...
		for (CollectionExtractionInfo info : CollectionExtractorProviderRegistry.instance()
				.getCollectionExtractionInfo()) {
	...
	/**
	 * Extracts from the heap the content of objects which represent a collection. The proper extractor is determined
	 * internally
	 *
	 * @param collection
	 *            an IObject representing a collection for which to extract the contents
	...
	/**
	 * Extracts from the heap the content of objects which represent a collection. For objects of class specifiedClass
	 * the preferredExtractor will be used, otherwise the extractor is selected internally
	 *
	 * @param collection
	 *            an IObject representing a collection for which to extract the contents
	 * @param specificClass
	 ...
	 *
	 * @param collection
	 *            an IObject representing a List for which to extract the contents
	...
	/**
	 * Extracts from the heap the content of objects which represent a Map. The proper extractor is determined
	 * internally
	 *
	 * @param collection
	 *            an IObject representing a Map for which to extract the contents
	...
	/**
	 * Extracts from the heap the content of objects which represent a Map. For objects of class specifiedClass the
	 * preferredExtractor will be used, otherwise the extractor is selected internally
	 *
	 * @param collection
	 *            an IObject representing a Map for which to extract the contents
	 * @param specificClass
