			...
			if (!foundFinalizer) {
				...
				for (IObject object : localVars) {
					if ((object.getObjectId() != i) // shouldn't be the
							...
							&& !object.getClazz().getName().equals("java.lang.ref.ReferenceQueue") // exclude //$NON-NLS-1$
																									// the queue also
							&& !object.getClazz().getName().equals("java.lang.ref.ReferenceQueue$Lock")) // exclude //$NON-NLS-1$
																											// also the
																											// queue
																											// lock
					{
