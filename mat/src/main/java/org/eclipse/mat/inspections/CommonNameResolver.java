...
public class CommonNameResolver {
	...
	/**
	 * For Oracle VMs for int.class, byte.class, void.class etc. These are just simple IObjects, not IClass objects. All
	 * other classes resolve via IClass.
	 */
	...
	public static class StackTraceElementResolver implements IClassSpecificNameResolver {
		public String resolve(IObject obj) throws SnapshotException {
			...
			if (fn == null)
				...
				name = cls.getClassSpecificName() + "." //$NON-NLS-1$
						+ methodName.getClassSpecificName()
						+ "() (" //$NON-NLS-1$
