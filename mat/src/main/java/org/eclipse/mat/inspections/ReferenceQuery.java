...
public class ReferenceQuery implements IQuery {
	...
	public IResult execute(IProgressListener listener) throws Exception {
					...
					if (ref != null) {
						try {
							...
							listener.sendUserMessage(IProgressListener.Severity.WARNING,
									MessageUtil.format(Messages.ReferenceQuery_MissingReferentObject,
											Long.toHexString(ref.getObjectAddress()),
											Long.toHexString(o.getObjectAddress()),
											referent_attribute),
									e);
						}
					}
		...
		return execute(instanceSet,
				referentSet,
				snapshot,
				Messages.ReferenceQuery_HistogramOfReferentObjects,
				Messages.ReferenceQuery_OnlyRetainedByReferents,
				Messages.ReferenceQuery_StronglyRetainedByReferences,
				referent_attribute,
				listener);
	}

	/**
	 * Important: the <strong>className</strong> must point to java.lang.ref.Reference or one of its subclasses. It is
	 * not possible to check this, as some heap dumps lack class hierarchy information.
	 */
	public static IResult execute(String className,
		...
		if (classes == null)
			throw new SnapshotException(
					MessageUtil.format(Messages.ReferenceQuery_ErrorMsg_NoMatchingClassesFound, className));
		...
		return execute(instanceSet,
				referentSet,
				snapshot,
				labelHistogramReferenced,
				labelHistogramRetained,
				labelHistogramStronglyRetainedReferents,
				DEFAULT_REFERENT,
				listener);
	}
	...
	private static boolean isReferenceType(IClass clazz) throws SnapshotException {
		return REFERENCE_PATTERN.matcher(clazz.getName()).matches() || "java.lang.ref.Finalizer".equals(clazz.getName()) //$NON-NLS-1$
