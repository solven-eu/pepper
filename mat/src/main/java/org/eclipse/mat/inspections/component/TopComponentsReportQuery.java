...
public class TopComponentsReportQuery implements IQuery {
	...
	public IResult execute(IProgressListener listener) throws Exception {
		SimpleMonitor sm = new SimpleMonitor(Messages.TopComponentsReportQuery_TopComponentReports,
				listener,
				new int[] { 10, 90 });
		...
		SimpleMonitor sm2 =
				new SimpleMonitor(Messages.TopComponentsReportQuery_TopComponentReports, sm.nextMonitor(), tasks);
		...
		for (Record record : loaders) {
			...
			/*
			 * Set a command which generates the same report for a class loader Uses OQL to select from the top level
			 * dominators, choosing an object which is the class loader, or is a class and is loaded by the loader or is
			 * an ordinary object of type of of a class loaded by the class loader.
			 */
	...
	private String oqlLoader(String name, long loaderAddress) {
		...
		"select * /* " + name.replace("*/", "* /") //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				+ " */ from objects (select objects a from objects (dominators(-1)) a) b" //$NON-NLS-1$
				+ " where"
				+ " b implements org.eclipse.mat.snapshot.model.IClassLoader and b.@objectAddress = "
				+ loaderAddress
				+ "L or" //$NON-NLS-1$
				+ " b implements org.eclipse.mat.snapshot.model.IClass and b.@classLoaderAddress = "
				+ loaderAddress
				+ "L or" //$NON-NLS-1$
				+ " b implements org.eclipse.mat.snapshot.model.IClassLoader = false and b implements org.eclipse.mat.snapshot.model.IClass = false and b.@clazz.@classLoaderAddress = "
				+ loaderAddress
				+ "L"; //$NON-NLS-1$
		// " $ {snapshot}.isClass(b.@objectId) and b.@classLoaderAddress = " + loaderAddress + "L or"+ //$NON-NLS-1$
		// //$NON-NLS-2$
		// " $ {snapshot}.isClassLoader(b.@objectId) = false and $ {snapshot}.isClass(b.@objectId) = false and
		// b.@clazz.@classLoaderAddress = " + loaderAddress + "L"; //$NON-NLS-1$ //$NON-NLS-2$
	}
