...
/**
 * This enumeration specifies how to display a number of bytes. It can be configured using
 * -Dbytes_display=(bytes|kilobytes|megabytes|gigabytes|smart) or through the Eclipse preferences dialog.
 *
 * @since 1.5
 */
public enum BytesDisplay {
	...
	/**
	 * If the value is a gigabyte or more, display in gigabytes; similarly for megabytes and kilobytes; otherwise,
	 * display in bytes.
	 */
	Smart;
	...
	/**
	 * We store the preference value statically to avoid constantly looking it up.
	 */
	 ...
	 * @return Given a stored preference value, return the enumeration value, or otherwise the default.
	 */
	...
	static {
		...
		if (propValue == null) {
			...
			String prefValue =
					Platform.getPreferencesService().getString(MATUI_PLUGIN, BytesDisplay.PROPERTY_NAME, null, null);
