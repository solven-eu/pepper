...
public final class Quantize {
	...
	/**
	 * A function used to aggregate values into one bucket, e.g. adding or averaging numbers. There exists one instance
	 * per bucket.
	 */
	...
	/**
	 * Creates a quantize {@link Builder} for a value distribution, i.e. rows are grouped by identical values. Rows can
	 * be grouped by one ore more values.
	 *
	 * @param label
	 *            multiple labels used to construct columns
	...
	/**
	 * Creates a quantize {@link Builder} for a value distribution, i.e. rows are grouped by identical values. Rows can
	 * be grouped by one ore more values. This constructor uses the given columns and their formatting and alignment to
	 * display the results.
	 *
	 * @param column
	 *            multiple columns, already with labels
	 * @return a Builder so that multiple construction options can be chained before calling {@link Builder#build()}
	 */
	...
	/**
	 * Creates a quantize {@link Builder} for a linear frequency distribution on double values.
	 * <p>
	 * Basically, one can answer questions like how many collections have a fill ratio between 0 and 20%, between 20%
	 * and 40%, etc.
	 *
	 * @param label
	...
	public static Builder linearFrequencyDistribution(String label, double lowerBound, double upperBound, double step) {
		...
		return new Builder(new Quantize(new KeyCalculator.LinearDistributionDouble(lowerBound, upperBound, step)))
				.addKeyColumn(new Column(label, Double.class).formatting(format).noTotals());
	}

	/**
	 * Creates a quantize {@link Builder} for a linear frequency distribution on long values.
	 * {@link Quantize#linearFrequencyDistribution(String, double, double, double)}
	 *
	 * @param label
	 *            How to label this quantization
	 * @param lowerBound
	 *            the lowest long value
	 * @param upperBound
	 *            the highest long value
	 * @param step
	 *            the interval for each bucket between lowerBound and upperBound
	 * @return a Builder so that multiple construction options can be chained before calling {@link Builder#build()}
	 */
	public static Builder linearFrequencyDistribution(String label, long lowerBound, long upperBound, long step) {
		...
		return new Builder(new Quantize(new KeyCalculator.LinearDistributionLong(lowerBound, upperBound, step)))
				.addKeyColumn(new Column(label, Long.class).formatting(format).noTotals());
	}
	...
	/* package */final static class QuantizedResult implements IResultTable {
		...
		private QuantizedResult(ResultMetaData resultMetaData,
				List<BucketImpl> values,
				List<Column> columns,
				int keyLength) {
