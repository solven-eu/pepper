...
public abstract class Filter {
	 ...
	 * is where approximate retained sizes are stored as negative numbers, but need the positive value for display. See
	 * {@link Column#setData(Object, Object)} and see {@link Column#getData(Object)}, as used in
	 * {@link org.eclipse.mat.query.ContextDerivedData#columnFor(org.eclipse.mat.query.ContextDerivedData.DerivedColumn, org.eclipse.mat.query.IResult, org.eclipse.mat.query.ContextProvider)}
	 */
		...
		public static Filter build(Column column, FilterChangeListener listener) {
			if (column.isNumeric()) {
				...
				boolean isPercentage =
						formatter instanceof DecimalFormat && ((DecimalFormat) formatter).toPattern().indexOf('%') >= 0;
	...
	public static final String[] FILTER_TYPES =
			new String[] { Messages.Filter_Label_Regex, Messages.Filter_Label_Numeric };
	...
	private static class NumericFilter extends Filter {
		...
		boolean accept(Object value) {
			...
			double doubleValue = value instanceof Bytes ? ((Bytes) value).getValue()
					: value instanceof Number ? ((Number) value).doubleValue() : Double.NaN;
			if (converter != null)
		...
		public boolean setCriteria(String criteria) throws IllegalArgumentException {
			...
			try {
				...
				throw new IllegalArgumentException(
						MessageUtil
								.format(Messages.Filter_Error_Parsing2, format(lowerExample()), format(upperExample()))
								+ e.getMessage(),
						e);
