...
public class CommandLine {
			...
			if (arg != null && arg.charAt(0) == '-' && arg.length() > 1) {
				...
				if (argDescriptor == null)
					throw new SnapshotException(
							MessageUtil.format(Messages.CommandLine_Error_MissingArgument, descriptor.getName(), flag));
				...
				if (argDescriptor.getType() == Boolean.class || argDescriptor.getType() == boolean.class) {
					...
					value = arguments.getQueryContext()
							.parse(argDescriptor.getType(), argDescriptor.getAdvice(), args, pos);
				...
				if (argDescriptor == null)
					throw new SnapshotException(
							MessageUtil.format(Messages.CommandLine_Error_NoUnflaggedArguments, arg));
	...
	private static Object consumeSingleArgument(IQueryContext context,
	...
	private static Object consumeMultipleArguments(IQueryContext context,
			ArgumentDescriptor descriptor,
			String[] args,
			ParsePosition pos) throws SnapshotException {
