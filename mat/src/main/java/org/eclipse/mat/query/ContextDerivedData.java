...
public abstract class ContextDerivedData {
	...
	public static final class DerivedColumn {
		...
		/**
		 * A derived column. Operations might include ways of calculating the column, such as approximate or precise
		 * retained sizes.
		 *
		 * @param label
	...
	public static final class DerivedOperation {
		 ...
		 *
		 * @param code
		 *            a text note of what the operation does (not translated)
		 * @param label
	...
	public interface DerivedCalculator {
		...
		/**
		 * Do the calculation for the row. Save the result in the row or elsewhere, ready for {@link #lookup(Object)}
		 *
		 * @param operation
		 *            the operation to do on the row to get the derived data
		 * @param row
		 *            the row
		 * @param listener
	 ...
	 *            the extra column
	 * @param result
	 *            the original result to be enhanced
	 * @param provider
	 ...
	 *
	 * @param operation
	 *            the operation to generate the column values
	 * @return the column
	 */
	public final DerivedColumn lookup(DerivedOperation operation) {
		...
		throw new IllegalArgumentException(MessageUtil.format(Messages.ContextDerivedData_Error_OperationNotFound,
				operation.getLabel(),
