...
public class QueryRegistry extends RegistryReader<IQuery> {
	...
	protected IQuery createDelegate(IConfigurationElement configElement) throws CoreException {
		try {
			...
			if (ReportPlugin.getDefault().isDebugging())
				ReportPlugin.log(IStatus.INFO,
						MessageUtil.format(Messages.QueryRegistry_Msg_QueryRegistered, descriptor));

			rootCategory = null;
			return descriptor != null ? query : null;
		} catch (SnapshotException e) {
			throw new CoreException(new Status(IStatus.ERROR,
					ReportPlugin.PLUGIN_ID,
					MessageUtil.format(Messages.QueryRegistry_Error_Registering, configElement.getAttribute("impl")), //$NON-NLS-1$
					e));
		}
	}
	...
	private final synchronized QueryDescriptor registerQuery(IQuery query) throws SnapshotException {
		...
		if (commandsByIdentifier.containsKey(identifier))
			throw new SnapshotException(MessageUtil.format(Messages.QueryRegistry_Error_NameBound,
					identifier,
		...
		/*
		 * $nl$ and annotations.properties .icon substitution added with 1.3 This allows
		 *
		 * @Icon("$nl$/META-INF/icons/myicon.gif") also this which is safe (ignored) for < V1.3 myquery.icon =
		 * $nl$/META-INF/icons/myicon.gif
		 */
		...
		QueryDescriptor descriptor =
				new QueryDescriptor(identifier, name, category, queryClass, usage, icon, help, helpUrl, helpLoc);
	...
	private Bundle findBundle(Class<?> cls) {
		 ...
		 *
		 * if (cl instanceof BundleReference) { BundleReference br = (BundleReference) cl; Bundle bb = br.getBundle();
		 * return bb; }
		 */
	...
	private void readMenuEntries(Class<? extends IQuery> queryClass, QueryDescriptor descriptor, ResourceBundle i18n) {
		...
		for (Menu.Entry entry : menu.value()) {
			...
			if (label == null || label.length() == 0)
				label = MessageUtil
						.format(Messages.QueryRegistry_MissingLabel, queryClass.getName(), String.valueOf(index + 1));
		...
		for (Field field : fields) {
			try {
				...
				throw new SnapshotException(
						MessageUtil.format(Messages.QueryRegistry_Error_Argument, field.getName(), clazz.getName()),
						e);
			}
		}
	}
	...
	private ArgumentDescriptor fromAnnotation(Class<?> clazz, Argument annotation, Field field, Object defaultValue)
		...
		if (advice == Argument.Advice.CLASS_NAME_PATTERN && !Pattern.class.isAssignableFrom(d.getType())) {
			String msg = MessageUtil.format(Messages.QueryRegistry_Error_Advice,
					field.getName(),
					clazz.getName(),
