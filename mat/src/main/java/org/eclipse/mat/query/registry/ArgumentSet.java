		...
		try {
			...
			for (ArgumentDescriptor parameter : query.getArguments()) {
				...
				if (value == null && parameter.isMandatory()) {
					...
					if (value == null)
						throw new SnapshotException(MessageUtil
								.format(Messages.ArgumentSet_Error_MissingMandatoryArgument, parameter.getName()));
				}
					...
					if (values.containsKey(parameter)) {
						if (ReportPlugin.getDefault().isDebugging()) {
							Logger.getLogger(getClass().getName())
									.log(Level.INFO,
											MessageUtil.format(Messages.ArgumentSet_Msg_NullValue,
													parameter.getName()));
						}
				...
				try {
					...
					throw new SnapshotException(MessageUtil.format(Messages.ArgumentSet_Error_IllegalArgument,
							value,
					...
					// registering queries
					throw new SnapshotException(MessageUtil.format(Messages.ArgumentSet_Error_Inaccessible,
							parameter.getName(),
							parameter.getType().getName()), e);
				}
			}
			...
			throw new SnapshotException(
					MessageUtil.format(Messages.ArgumentSet_Error_Instantiation, query.getCommandType().getName()),
					e);
		} catch (IllegalAccessException e) {
			throw new SnapshotException(
					MessageUtil.format(Messages.ArgumentSet_Error_SetField, query.getCommandType().getName()),
					e);
	...
	public void setArgumentValue(String name, Object value) {
		...
		if (argument == null)
			throw new RuntimeException(
					MessageUtil.format(Messages.ArgumentSet_Error_NoSuchArgument, query.getIdentifier(), name));
