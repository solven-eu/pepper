...
public class HprofParserHandlerImpl implements IHprofParserHandler {
	...
	public void beforePass2(IProgressListener monitor) throws IOException, SnapshotException {
		...
		outbound = new IntArray1NWriter(this.identifiers.size(), Index.OUTBOUND.getFile(info.getPrefix() + "temp."));//$NON-NLS-1$
		object2classId = new IntIndexCollector(this.identifiers.size(), IndexWriter.mostSignificantBit(maxClassId));
		object2position =
				new LongIndexCollector(this.identifiers.size(), IndexWriter.mostSignificantBit(maxFilePosition));
		...
		HeapObject heapObject = new HeapObject(0, classLoaderClass, classLoaderClass.getHeapSizePerInstance());
	...
	/**
	 * Possible HPROF extension: classes also with instance dump records. The classes could be of a type other than
	 * java.lang.Class There could also be per-instance fields defined by their type. Those values can be made
	 * accessible to MAT by creating pseudo-static fields.
	 */
		...
		for (Iterator<Entry<ClassImpl>> it = classesByAddress.entries(); it.hasNext();) {
			...
			if (type != null) {
				...
				if (newStatics.size() != cl.getStaticFields().size()) {
					ClassImpl newcl = new ClassImpl(cl.getObjectAddress(),
							cl.getName(),
							cl.getSuperClassAddress(),
							cl.getClassLoaderAddress(),
							newStatics.toArray(new Field[newStatics.size()]),
			...
			for (long arrayClassID : requiredArrayClassIDs.keySet()) {
				...
				if (arrayType == null) {
					...
					arrayType = new ClassImpl(arrayClassID,
							"unknown-class-" + clsid + "[]", //$NON-NLS-1$ //$NON-NLS-2$
							jlo,
							0,
	...
	public IOne2LongIndex fillIn(IPreliminaryIndex index, IProgressListener listener) throws IOException {
		/*
		 * System classes should be marked appropriately in the HPROF file. lambda classes should not be marked as
		 * system GC roots.
		 */
		...
		for (ClassImpl clazz : discardedObjectsByClass.values()) {
			records.add(new UnreachableObjectsHistogram.Record(clazz.getName(),
		...
		if (discardedObjects > 0) {
			...
			listener.sendUserMessage(IProgressListener.Severity.WARNING,
					MessageUtil.format(Messages.HprofParserHandlerImpl_DiscardedObjects,
							discardedObjects,
							discardedSize,
							discardRatio,
							discardPattern),
		...
		index.setIdentifiers((new LongIndexStreamer()).writeTo(Index.IDENTIFIER.getFile(info.getPrefix() + "temp."), //$NON-NLS-1$
	...
	private void prepareHeapObject(HeapObject object) throws IOException {
		...
		if (!object.isObjectArray && !object.isPrimitiveArray) {
			...
			for (IClass clazz : hierarchy) {
				for (FieldDescriptor field : clazz.getFieldDescriptors()) {
					...
					for (int stidx = 0; stidx < statics.length; ++stidx) {
						if (statics[stidx] != null && statics[stidx].getType() == type
								&& statics[stidx].getName().equals("<" + field.getName() + ">")) { //$NON-NLS-1$ //$NON-NLS-2$
							// Found a field
			...
			if (pos != object.instanceData.length) {
				...
				// if (unknown && (strictnessPreference == HprofStrictness.STRICTNESS_WARNING || strictnessPreference ==
				// HprofStrictness.STRICTNESS_PERMISSIVE))
				// {
				// //monitor.sendUserMessage(Severity.WARNING,
				// MessageUtil.format(Messages.Pass2Parser_Error_InsufficientBytesRead, thisClazz.getName(),
				// Long.toHexString(id), Long.toHexString(segmentStartPos), Long.toHexString(endPos),
				// Long.toHexString(in.position())), null);
				// }
				// else
				// {
				// throw new IOException(MessageUtil.format(Messages.Pass2Parser_Error_InsufficientBytesRead,
				// thisClazz.getName(), Long.toHexString(id), Long.toHexString(segmentStartPos),
				// Long.toHexString(endPos), Long.toHexString(in.position())));
				// }
			}
		}
	...
	private boolean discard() {
		...
		/*
		 * Wrap around the range. [dddddddd..] 0.8,0.0 [..dddddddd] 0.8,0.2 [dd..dddddd] 0.8,0.4
		 */
	...
	/**
	 * Adjust the last seen file position. Used to see how many bits are needed for the index.
	 *
	 * @param filePosition
	 */
