...
public class HprofRandomAccessParser extends AbstractParser {
	...
	public synchronized IObject read(int objectId, long position, ISnapshot dump, IOne2LongIndex o2hprof)
	...
	/**
	 * A reference to an object via its address which can handle the address not being in the index.
	 */
	static class ObjectAddressReference extends ObjectReference {
		...
		/**
		 * Construct an {@link ObjectAddressReference} which can be used to retrieve an object by address even if it has
		 * not been indexed. Used as a proxy by {@link ObjectReference} which can change the
		 * {@link ObjectReference#address} private field.
		 *
		 * @param snapshot
		 *            the snapshot
		 * @param parser
		 *            the HPROF parser used to build the IObject if required
		 * @param o2hprof
		 *            the index from object ID to HPROF file position
		 * @param address
		 *            the address of the object
		 */
		public ObjectAddressReference(ISnapshot snapshot,
				HprofRandomAccessParser parser,
				IOne2LongIndex o2hprof,
				long address) {
		...
		/**
		 * Actually construct an IObject. First try constructing by object ID. If that fails then construct by address.
		 * Find the indexed objects before and after this. Parse the HPROF file between these two points looking for an
		...
		public IObject getObject() throws SnapshotException {
			...
			if (pos == 0) {
				...
				if (lpos != null)
					...
					while ((pos = o2hprof.get(low)) == 0 && low < o2hprof.size() - 1)
						++low;
			}
			long posHigh = pos;
