...
/**
 * Uses a temporary file for caching the unzipped version of the original file. Faster than
 * {@link CompressedRandomAccessFile} but uses lots of disk space. Package class. Do not call any methods other than
 * {@link #seek(long)} {@link #getFilePointer()} {@link #length()} - not necessarily accurate {@link #read(byte[])}
 * {@link #read(byte[], int, int)} {@link #close()}
 */
class FileCacheCompressedRandomAccessFile extends RandomAccessFile {
	ImageInputStream iis;

	/**
	 * Create an unzipped view of the gzipped file, using a file cache for the uncompressed data.
	 *
	 * @param file
	 * @throws IOException
	 */
	 ...
	 *            The gzipped file.
	 * @param len
	 *            the unzipped length, or -1 if unknown
