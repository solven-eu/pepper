...
public class BufferingRafPositionInputStream implements IPositionInputStream, Closeable, AutoCloseable {
	...
	public BufferingRafPositionInputStream(final File file,
			final String prefix,
			final long offset,
	...
	public long skip(long n) throws IOException {
		...
		if (n > readLength) {
			/*
			 * If n is too big then this would allow a seek beyond the end of the file.
			 */
			seek(position() + n);
			return n;
		}
	...
	public long readLong() throws IOException {
		if (!ensureAvailable(8))
			throw new EOFException();
		long result = (((long) buffer[bufferPosition] << 56) + ((long) (buffer[bufferPosition + 1] & 0xff) << 48)
