...
/**
 * Creates an unzipped view of a Gzipped file. Probably quite slow for random access, okay for streaming access. Package
 * class. Do not call any methods other than {@link #seek(long)} {@link #getFilePointer()} {@link #length()} - not
 * necessarily accurate {@link #read(byte[])} {@link #read(byte[], int, int)} {@link #close()}
 */
class CompressedRandomAccessFile extends RandomAccessFile {
	SeekableStream ss;

	/**
	 * Create an unzipped view of the gzipped file, using multiple gzipped readers to obtain the uncompressed data.
	 *
	 * @param file
	 *            the compressed file
	 * @param random
	...
	public CompressedRandomAccessFile(File file, boolean random, long length) throws IOException {
		...
		ss = new SeekableStream(new Supplier<InputStream>() {
			public InputStream get() {
				try {
					/*
					 * Create a stream view of the channel. Important - changing position via channel must change
					 * position of input stream, so no buffering. Add mark support.
					 */
	...
	/**
	 * Estimate the length of a file, including GZip without decompressing the whole file.
	 *
	 * @param f
	...
	/**
	 * Estimates the work to do when reading the file. For compressed files it is not easy to do this based on the
	 * original size, since it is not generally known. Instead for these we use the physical size and the physical read
	...
	/**
	 * Estimate the length of the uncompressed version of the Gzipped file. Gzip only has the least significant 32-bits
	 * of the size. Estimate the size as 5.0 times the compressed size, but with the same least significant 32-bits.
	 *
	 * @param ra
	 *            The file
	...
	/**
	 * Check whether there is at least the requested amount of memory available.
	 *
	 * @param requested
