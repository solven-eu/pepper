...
public class ByteArrayPositionInputStream implements IPositionInputStream, Closeable, AutoCloseable {
	/*
	 * Can throw EOFExceptions. If you use it wrong, it will throw ArrayIndexOutOfBoundsException.
	 */
	...
	public long readLong() throws EOFException {
		if (position > bytes.length - 4)
			throw new EOFException();
		long result = (((long) bytes[position] << 56) + ((long) (bytes[position + 1] & 0xff) << 48)
