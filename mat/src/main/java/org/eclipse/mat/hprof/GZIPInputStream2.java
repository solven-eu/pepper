...
/**
 * A stream for reading Gzip compressed files. Uses a modified {@link io.nayuki.deflate.InflaterInputStream} to allow
 * the state of the stream to be duplicated, aiding code for random access to Gzip files.
 */
public class GZIPInputStream2 extends FilterInputStream {
	...
	/**
	 * Copy constructor used for duplicating a stream. To use either stream thereafter the underlying stream must be in
	 * the correct position.
	 *
	 * @param gs
	 *            the stream to be duplicated
	 * @throws IOException
	 */
	...
	private InputStream readHeader(InputStream is, int b0) throws IOException {
		...
		if ((b3 & FLG_FHCRC) != 0) {
			...
			if ((crc.getValue() & 0xffff) != crcv)
				throw new ZipException(MessageUtil.format(Messages.GZIPInputStream2_BadHeaderCRC,
						Integer.toHexString(crcv),
						Integer.toHexString((int) (crc.getValue() & 0xffff))));
		}
		crc.reset();
		return is;
	}
	...
	/**
	 * Need to implement {@link FilterInputStream#skip(long)} to the check CRC. We can call the underlying
	 * {@link #skip(long)} if we are rereading data.
	 *
	 * @param n
	 *            bytes to skip
	...
	void checkTrailer(InputStream is) throws IOException {
		...
		if (crc32v != crc32)
			throw new ZipException(MessageUtil.format(Messages.GZIPInputStream2_BadTrailerCRC,
					Integer.toHexString((int) crc32),
					Integer.toHexString((int) crc32v)));
		// uncompressed length
		int b4 = is.read();
		if (b4 < 0)
			throw new EOFException();
		...
		if (len32 != (uncompressedLen & 0xffffffffL))
			throw new ZipException(MessageUtil.format(Messages.GZIPInputStream2_BadTrailerLength,
					Integer.toHexString((int) len32),
					Long.toHexString(uncompressedLen)));
	}
	...
	/**
	 * A CRC32 implementation - which allows the state to be cloned.
	 */
