...
public class HistogramBuilder extends HistogramRecord {
	...
	public Histogram toHistogram(SnapshotImpl snapshot, boolean isDefaultHistogram) throws SnapshotException {
		...
		HashMapIntObject<ClassLoaderHistogramRecordBuilder> cl2builder =
				new HashMapIntObject<ClassLoaderHistogramRecordBuilder>();
		...
		for (Iterator<?> e = data.values(); e.hasNext();) {
			...
			if (clRec == null) {
				...
				long retainedHeapSize = isDefaultHistogram
						? ClassLoaderImpl.doGetRetainedHeapSizeOfObjects(snapshot, classLoaderId, false, false, null)
						: 0;
		...
		ArrayList<ClassLoaderHistogramRecord> classLoaderHistogramRecords =
				new ArrayList<ClassLoaderHistogramRecord>(cl2builder.size());
