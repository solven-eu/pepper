	...
	public static class FromClause {
		...
		public String toString() {
			...
			if (subSelect != null) {
				...
				/*
				 * Ensure that a non-env var or constant has parentheses so it isn't confused with a FROM classname Add
				 * parentheses for select wrapped as a call expression, otherwise will be reparsed just as a subselect (
				 * select * from objects ${snapshot} ) Add parentheses for this: select * from objects (1) Add
				 * parentheses for this: select * from objects (var) No need to add parentheses as plus will add some:
