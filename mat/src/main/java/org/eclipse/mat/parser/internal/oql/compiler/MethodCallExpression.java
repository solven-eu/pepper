...
class MethodCallExpression extends Expression {
	...
	public Object compute(EvaluationContext ctx) throws SnapshotException, OperationCanceledException {
		...
		/*
		 * Finding the right method is tricky as the arguments have already been boxed. E.g. consider overloaded methods
		 * remove(int) remove(Object) with argument Integer(1).
		 */
			...
			if (methods[ii].getName().equals(this.name)) {
				...
				if (parameterTypes.length == arguments.length
					...
					for (int jj = 0; jj < arguments.length; jj++) {
						...
						if (!(methods[ii].isVarArgs() && jj >= parameterTypes.length - 1) && arguments[jj] != null
								&& !isConvertible(parameterTypes[jj], arguments[jj])) {
							...
							if (parameterTypes[jj].isAssignableFrom(Pattern.class)) {
								...
								arguments[jj] =
										Pattern.compile(PatternUtil.smartFix(String.valueOf(arguments[jj]), false));
							} else {
								if (savedArgs != null) {
									// Restore arguments
	...
	/**
	 * Try for a good match on the method. Also try unboxed arguments.
	 */
	private void firstChoiceMethods(List<Method> extraMethods,
			final Class<? extends Object> subjectClass,
			Object[] arguments) {
	...
	private void extracted(List<Method> extraMethods,
			final Class<? extends Object> subjectClass,
	...
	private boolean isConvertible(Class<?> parameterType, Object argument) {
		...
		if (parameterType.isAssignableFrom(argumentType))
			return true;
		if (argumentType == Boolean.class && (parameterType == boolean.class || parameterType == Boolean.class))
			return true;
		if (argumentType == Byte.class && (parameterType == byte.class || parameterType == Byte.class
				...
				|| parameterType == Double.class))
			return true;
		if (argumentType == Short.class && (parameterType == short.class || parameterType == Short.class
				|| parameterType == int.class
				|| parameterType == Integer.class
				|| parameterType == long.class
				|| parameterType == Long.class
				...
				|| parameterType == Double.class))
			return true;
		if (argumentType == Character.class && (parameterType == char.class || parameterType == Character.class
				|| parameterType == int.class
				|| parameterType == Integer.class
				|| parameterType == long.class
				|| parameterType == Long.class
				...
				|| parameterType == Double.class))
			return true;
		if (argumentType == Integer.class && (parameterType == int.class || parameterType == Integer.class
				|| parameterType == long.class
				|| parameterType == Long.class
				|| parameterType == float.class
				|| parameterType == Float.class
				...
				|| parameterType == Double.class))
			return true;
		if (argumentType == Long.class && (parameterType == long.class || parameterType == Long.class
				|| parameterType == float.class
				|| parameterType == Float.class
				|| parameterType == double.class
				|| parameterType == Double.class))
			return true;
		if (argumentType == Float.class && (parameterType == float.class || parameterType == Float.class
				|| parameterType == double.class
				|| parameterType == Double.class))
			return true;
		if (argumentType == Double.class && (parameterType == double.class || parameterType == Double.class))
			return true;
		return false;
	}

	/**
	 * Check whether to allow this method call. Syntax for mat.oqlmethodFilter com.package1.* only classes/methods in
	 * this package com.package1.** classes/methods in this package or subpackages com.package* classes/methods starting
	 * with the prefix com.*#methodname prefix before * and suffix after * must match ! means not allowed ; separates
	 * components Throws an exception if not allowed.
	 */
	static void checkMethodAccess(Method method) {
		...
		for (String pt : match.split(";")) //$NON-NLS-1$
		{
			...
			if (pt.endsWith(".**")) //$NON-NLS-1$
				...
				m = nm.startsWith(pt.substring(0, pt.length() - 1)) && !nm.substring(pt.length() - 1).contains("."); //$NON-NLS-1$
			else if (pt.endsWith("*")) //$NON-NLS-1$
			...
			if (not && m)
				throw new AccessControlException(
						MessageUtil.format(Messages.MethodCallExpression_Error_MethodProhibited, nm, "!" + pt, match)); //$NON-NLS-1$
			if (m)
				break;
		}
	}
	...
	private Object[] convertVarArgs(Class<?>[] parameterTypes, Object arguments[]) {
		/*
		 * If there is one var args argument and it looks like it matches the object array, then don't wrap it.
		 */
		if (!(arguments.length == parameterTypes.length
			...
			Object varargs[] =
					(Object[]) Array.newInstance(componentType, arguments.length - (parameterTypes.length - 1));
