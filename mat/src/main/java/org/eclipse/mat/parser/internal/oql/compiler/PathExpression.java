						...
						if (current.getClass().isArray()) {
							if ("length".equals(attribute.getName())) //$NON-NLS-1$
							{
								...
								throw new SnapshotException(
										MessageUtil.format(Messages.PathExpression_Error_ArrayHasNoProperty,
												new Object[] { current.getClass().getComponentType().getName(),
														attribute.name }));
							}
						} else {
								...
								for (PropertyDescriptor descriptor : descriptors) {
									if (attribute.getName().equals(descriptor.getName())) {
										...
										for (Class<?> intf : method.getDeclaringClass().getInterfaces()) {
											try {
												// Avoid some NoSuchMethodExceptions by checking the name and number of
												// parms first
												for (Method m3 : intf.getMethods()) {
													...
													if (m3.getParameterCount() != method.getParameterTypes().length)
														continue;
													Method m2 = intf.getMethod(method.getName(),
										...
										if (!useInterface) {
											try {
												...
												throw new SnapshotException(
														MessageUtil
																.format(Messages.PathExpression_Error_TypeHasNoProperty,
							...
							if (!didFindProperty) {
								throw new SnapshotException(
										MessageUtil.format(Messages.PathExpression_Error_TypeHasNoProperty,
												new Object[] { current.getClass().getName(), attribute.name }));
							}
						}
