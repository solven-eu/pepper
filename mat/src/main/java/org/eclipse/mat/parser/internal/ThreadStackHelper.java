				...
				if (line.startsWith("Thread")) //$NON-NLS-1$
				{
							...
							if (lineNr >= 0) {
								...
								try {
									...
									logger.log(Level.WARNING,
											MessageUtil.format(Messages.ThreadStackHelper_InvalidThreadLocal,
													line.trim(),
													"0x" + Long.toHexString(threadAddress), //$NON-NLS-1$
					...
					if (threadAddress != -1) {
						try {
							...
							logger.log(Level.WARNING,
									MessageUtil.format(Messages.ThreadStackHelper_InvalidThread,
											"0x" + Long.toHexString(threadAddress), //$NON-NLS-1$
