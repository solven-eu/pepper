...
public class SnapshotFactoryImpl implements SnapshotFactory.Implementation {
	...
	private void validateIndices(PreliminaryIndexImpl pidx, IProgressListener listener) {
		...
		for (int i = 0; i < maxIndex; ++i) {
			...
			if (prevAddress == addr) {
				...
				listener.sendUserMessage(Severity.ERROR,
						MessageUtil.format(Messages.SnapshotFactoryImpl_IndexAddressHasSameAddressAsPrevious,
								i,
								desc,
								format(addr),
								desc2),
			...
			if (prevAddress > addr) {
				String desc = objDesc(pidx, i);
				listener.sendUserMessage(Severity.ERROR,
						MessageUtil.format(Messages.SnapshotFactoryImpl_IndexAddressIsSmallerThanPrevious,
								i,
								desc,
								format(addr),
								format(prevAddress)),
			...
			if (i != j) {
				...
				String desc2 = objDesc(pidx, j);
				listener.sendUserMessage(Severity.ERROR,
						MessageUtil.format(Messages.SnapshotFactoryImpl_IndexAddressFoundAtOtherID,
								i,
								format(addr),
								j,
								desc1,
			...
			if (clsId < 0) {
				listener.sendUserMessage(Severity.ERROR,
						MessageUtil.format(Messages.SnapshotFactoryImpl_ClassIDNotFound, i, format(addr), clsId),
						null);
			} else {
				ClassImpl ci = pidx.classesById.get(clsId);
				if (ci == null) {
					listener.sendUserMessage(Severity.ERROR,
							MessageUtil.format(Messages.SnapshotFactoryImpl_ClassImplNotFound, i, format(addr), clsId),
							null);
				}
			}
			...
			if (outs == null) {
				String desc = objDesc(pidx, i);
				listener.sendUserMessage(Severity.ERROR,
						MessageUtil.format(Messages.SnapshotFactoryImpl_NoOutbounds, i, format(addr), desc),
						null);
			} else {
				if (outs.length == 0) {
					String desc = objDesc(pidx, i);
					listener.sendUserMessage(Severity.ERROR,
							MessageUtil.format(Messages.SnapshotFactoryImpl_EmptyOutbounds, i, format(addr), desc),
							null);
				} else {
					for (int k = 0; k < outs.length; ++k) {
						if (outs[k] < 0 || outs[k] >= maxIndex) {
							String desc = objDesc(pidx, i);
							listener.sendUserMessage(Severity.ERROR,
									MessageUtil.format(Messages.SnapshotFactoryImpl_InvalidOutbound,
											i,
											format(addr),
											desc,
											k,
			...
			if (ci == null) {
				...
				if (size < 0) {
					...
					listener.sendUserMessage(Severity.ERROR,
							MessageUtil.format(Messages.SnapshotFactoryImpl_IndexAddressNegativeArraySize,
									i,
									format(addr),
									size,
									ci.getTechnicalName()),
				...
				if (addr != addr2) {
					listener.sendUserMessage(Severity.ERROR,
							MessageUtil.format(Messages.SnapshotFactoryImpl_ClassIndexAddressNotEqualClassObjectAddress,
									i,
									format(addr),
									format(addr2),
									ci.getTechnicalName()),
				...
				if (i != id) {
					listener.sendUserMessage(Severity.ERROR,
							MessageUtil.format(Messages.SnapshotFactoryImpl_ClassIndexNotEqualClassObjectID,
									i,
									format(addr),
									id,
									ci.getTechnicalName()),
				...
				if (clsId != clsId2) {
					listener.sendUserMessage(Severity.ERROR,
							MessageUtil.format(
									Messages.SnapshotFactoryImpl_ClassIndexAddressTypeIDNotEqualClassImplClassId,
									i,
									format(addr),
									clsId,
									clsId2,
				...
				if (ldr < 0) {
					listener.sendUserMessage(Severity.ERROR,
							MessageUtil.format(Messages.SnapshotFactoryImpl_ClassIndexAddressNoLoaderID,
									i,
									format(addr),
									clsId,
									ldr,
		...
		if (nObjsFromClass != nObjs + nCls) {
			listener.sendUserMessage(Severity.ERROR,
					MessageUtil.format(Messages.SnapshotFactoryImpl_ObjectsFoundButClassesHadObjectsAndClassesInTotal,
							nObjs,
							nCls,
							nObjsFromClass),
					null);
		...
		for (IteratorInt it = pidx.gcRoots.keys(); it.hasNext();) {
			...
			if (idx < 0 || idx >= maxIndex) {
				listener.sendUserMessage(Severity.ERROR,
						MessageUtil.format(Messages.SnapshotFactoryImpl_GCRootIDOutOfRange, idx, maxIndex),
						null);
			} else {
		...
		for (int thrd : pidx.thread2objects2roots.getAllKeys()) {
			if (thrd < 0 || thrd >= maxIndex) {
				listener.sendUserMessage(Severity.ERROR,
						MessageUtil.format(Messages.SnapshotFactoryImpl_GCThreadIDOutOfRange, thrd, maxIndex),
						null);
			} else {
				...
				for (int idx : l.getAllKeys()) {
					if (idx < 0 || idx >= maxIndex) {
						listener.sendUserMessage(Severity.ERROR,
								MessageUtil.format(Messages.SnapshotFactoryImpl_GCThreadRootIDOutOfRange,
										thrd,
										idx,
										maxIndex),
								null);
						...
						for (XGCRootInfo ifo : l.get(idx)) {
							...
							if (objid != idx) {
								listener.sendUserMessage(Severity.ERROR,
										MessageUtil.format(Messages.SnapshotFactoryImpl_GCThreadRootIDDoesNotMatchIndex,
												thrd,
												objid,
												idx),
										null);
		...
		if (newObjId >= 0) {
			...
			if (classInfo != null) {
				...
				if (clsId >= 0 && clsId < pidx.identifiers.size()) {
					...
					if (classInfo != null) {
						clsInfo = MessageUtil.format(Messages.SnapshotFactoryImpl_ObjDescObjType,
								classInfo.getName(),
								format(clsAddr));
					} else {
						clsInfo =
	...
	private void deleteIndexFiles(File file, final String prefix, IProgressListener listener) {
		...
		File[] files = directory.listFiles(new FileFilter() {
			...
			public boolean accept(File f) {
				...
				return name.startsWith(fragment) && (indexPattern.matcher(name.substring(fragment.length())).matches()
			...
			for (File f : files) {
				...
				if (!deleted) {
					listener.sendUserMessage(Severity.WARNING,
							MessageUtil.format(Messages.SnapshotFactoryImpl_UnableToDeleteIndexFile, f.toString()),
							null);
				}
			}
		}
