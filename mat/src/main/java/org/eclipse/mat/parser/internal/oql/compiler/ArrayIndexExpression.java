...
class ArrayIndexExpression extends Expression {
	...
	private static class CollectionObjectSubList extends AbstractList<IObject> {
		...
		public IObject get(int index) {
			...
			if (i1 + index >= objs.length) {
				/*
				 * Currently the CollectionsExtractor only returns non-null entries, so the number of entries can be
				 * less than the size, so pad at the end with nulls.
				 */
				return null;
			}
	 ...
	 * @param s
	 * @param a
	 * @return [0][1:-1] -> [0,0) [1][1:-1] -> [0,1) [2][1:-1] -> [1,2)
	 */
