...
public abstract class TextEmitter {
	...
	/**
	 * Append content to be copied. Implementations should put the content to their own storage - buffer, file, etc...
	 *
	 * @param string
	 */
	...
	/**
	 * The method is called at the end of the copy procedure. It can be used by implementations to flush content, move
	 * to clipboard, etc...
	 */
	...
	/**
	 * Whether to process a child of a tree (display it and possibly examine its children.)
	 *
	 * @param child
	 * @return
	 */
	...
	public void doCopy() {
		...
		if (numberOfColumns == 0) {
			...
			for (int i = 0; i < order.length; i++) {
				...
				append(align(getColumnName(columns[col]),
						align[col],
						columnLengths.get(col),
						i + 1 == numberOfColumns));
			}
			...
			for (int j = 0; j < children.length; j++) {
				...
				if (shouldProcessChild(children[j])) {
					if (numberOfColumns < 1) {
						...
						append(level.toString());
						append(align(getDisplayableColumnValue(children[j], col),
								align[col],
								length - level.length(),
								numberOfColumns == 1));
	...
	/**
	 * Align a string in a field. Right aligned fields have a space either side of the longest entry
	 *
	 * @param s
	 *            the data
	 * @param alignment
	 *            An integer representing alignment which is seeded by the subclass as part of
	 *            {@link #findColumnOrderAndAlignment(Object[])} and interpreted using {@link #isAlignmentRight(int)}
	 *            and {@link #isAlignmentCenter(int)}
	 * @param length
	 *            the size of the field
	 * @param last
	 *            whether this is the last field in a line, so trailing spaces can be omitted
	 * @return
	 */
