...
public class QueryPart extends AbstractPart {
	...
	public AbstractPart execute(IQueryContext context, ResultRenderer renderer, IProgressListener listener)
		...
		if (result == null) {
			if (getCommand() == null) {
				ReportPlugin.log(IStatus.ERROR,
						MessageUtil.format(Messages.QueryPart_Error_NoCommand, //
								spec().getName(),
								sectionName));
			} else {
				try {
					...
					ReportPlugin.log(e,
							MessageUtil.format(Messages.QueryPart_Error_IgnoringResult, spec().getName(), msg));
		...
		for (String column : hidden) {
			...
			if (columnIndex < 0) {
				Logger.getLogger(getClass().getName())
						.log(Level.SEVERE, MessageUtil.format(Messages.QueryPart_Error_ColumnNotFound, column));
			} else {
	...
	/**
	 * Set a filter when the filter is not localized. Currently {@link RefinedResultBuilder#setFilter(int, String)}
	 * expects numbers to be localized. This is possible a problem for reports, which might not be fully localized. If a
	 * report sets a filter >=10.301 this will have a problem in a French locale where . is a thousands separator. For
	 * queries parse numbers in the English locale, and replace with the result in the column formatter locale.
	 *
	 * @param builder
	 * @param col
	 * @param filter
	...
	private void setFilter(RefinedResultBuilder builder, int col, String filter) {
		...
		if (fmt instanceof DecimalFormat) {
			...
			if (decSep != '.' && filter.indexOf('.') >= 0 || percent != '%' && filter.indexOf('%') >= 0
	...
	private void addFilter(RefinedResultBuilder builder) {
		...
		for (String filter : filters) {
			...
			if (p < 0) {
				Logger.getLogger(getClass().getName())
						.log(Level.SEVERE, MessageUtil.format(Messages.QueryPart_Error_MissingEqualsSign, filter));
			} else {
				int columnIndex = getColumnIndex(builder, filter.substring(0, p));
				if (columnIndex < 0) {
					Logger.getLogger(getClass().getName())
							.log(Level.SEVERE,
									MessageUtil.format(Messages.QueryPart_Error_ColumnNotFound,
											filter.substring(0, p)));
				} else {
					try {
						...
						Logger.getLogger(getClass().getName())
								.log(Level.SEVERE,
										MessageUtil.format(Messages.QueryPart_Error_Filter, filter.substring(p + 1)),
										e);
					}
				}
			}
	...
	/** format: filter1=criteria,filter2=criteria */
	/**
	 * format: contextprovider1=operation,contextprovider2=operation,_default_= operation
	 */
		...
		for (String provider : providers) {
			...
			if (p < 0) {
				ReportPlugin.log(IStatus.WARNING,
						MessageUtil.format(Messages.QueryPart_Error_InvalidProvider,
								Params.Rendering.DERIVED_DATA_COLUMN,
								provider));
				continue;
			}
			...
			if (operation == null) {
				ReportPlugin.log(IStatus.WARNING,
						MessageUtil.format(Messages.QueryPart_Error_InvalidProviderOperation,
								Params.Rendering.DERIVED_DATA_COLUMN,
								code));
				continue;
			}
	...
	private void addSortOrder(RefinedResultBuilder builder) {
		...
		for (String column : columns) {
			...
			if (columnIndex < 0) {
				Logger.getLogger(getClass().getName())
						.log(Level.WARNING, MessageUtil.format(Messages.QueryPart_Error_SortColumnNotFound, name));
				continue;
			}
