...
public class PartsFactory {
	...
	private AbstractPart build(AbstractPart parent, Spec spec) {
		...
		if (answer == null)
			throw new RuntimeException(
					MessageUtil.format(Messages.PartsFactory_Error_Construction, spec.getClass().getName()));
	...
	public AbstractPart createClone(AbstractPart template, Spec spec) {
		...
		if (spec instanceof SectionSpec)
			answer = new SectionPart(template
					.getId(), template.getParent(), artefact, (SectionSpec) spec, template.getCommand());
		else if (spec instanceof QuerySpec)
