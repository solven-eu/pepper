	...
	public void embedd(Context context, IResult result, Writer writer) throws IOException {
		if (result instanceof TextResult) {
			...
			for (Slice s : pie.getSlices()) {
				writer.append(MessageUtil
						.format(Messages.TextOutputter_Slice, sl, s.getValue(), toTextFromHTML(s.getDescription())));
				writer.append(LINE_SEPARATOR);
				writer.append(LINE_SEPARATOR);
				++sl;
			}
	...
	private String toTextFromHTML(String s) {
		{
			...
			s = s.replaceAll("<ul [^>]+>", LINE_SEPARATOR).replaceAll("</ul>", ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
																					// //$NON-NLS-4$
			s = s.replaceAll("<li>", "").replace("</li>", LINE_SEPARATOR); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
																			// //$NON-NLS-4$
	...
	private static abstract class StructuredResultTextEmitter extends TextEmitter {
		...
		protected void findColumnOrderAndAlignment(Object[] genericColumns) {
			...
			for (int columnIndex = 0; columnIndex < columns.length; columnIndex++) {
				...
				/*
				 * It's a bit complicated to handle invisible columns because the columnIndex may be used to lookup
				 * values. Omit the column from the ordering.
				 */
