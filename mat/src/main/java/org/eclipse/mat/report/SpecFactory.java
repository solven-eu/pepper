...
public final class SpecFactory extends RegistryReader<SpecFactory.Report> {
		...
		if (master.getTemplate() != null) {
			...
			if (other != null) {
				...
				Logger.getLogger(getClass().getName())
						.log(Level.SEVERE, MessageUtil.format(Messages.SpecFactory_Error_MissingTemplate, template));
			}
		}
	...
	private static class SpecHandler extends DefaultHandler {
		...
		/**
		 * Some values are translatable using % prefix and a translation in the plugin.properties file. %key.part
		 * default value 'key.part' is looked up, if missing then 'default value' is returned (without quotes).
		 *
		 * @param n
