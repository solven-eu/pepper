...
public class HtmlOutputter implements IOutputter {
	...
	// //////////////////////////////////////////////////////////////
	/**
	 * Extract alternate text for image URL. Use result within double quotes for attributes.
	 *
	 * @param url
	...
	private void renderTable(Context context, RefinedTable table, Writer artefact, boolean hasDetailsLink)
		...
		int numberOfRowsToDisplay =
				(context.hasLimit() && context.getLimit() < table.getRowCount()) ? context.getLimit()
						: table.getRowCount();
	...
	@SuppressWarnings("nls")
	private void renderFilterRow(Context context,
			Writer artefact,
			RefinedStructuredResult result,
	...
	@SuppressWarnings("nls")
	private void renderTotalsRow(Context context,
			Writer artefact,
			RefinedStructuredResult result,
			List<?> elements,
			TotalsRow totalsRow,
	...
	private void renderChildren(Context context,
			...
			boolean hasDetailsLink) throws IOException {
		int numberOfRowsToDisplay =
				(context.hasLimit() && context.getLimit() < elements.size()) ? context.getLimit() : elements.size();
	...
	private void renderDepthRow(Writer artefact) throws IOException {
		...
		artefact.append("<td style=\"padding-left:1000px\">") //
				.append("&raquo; " + Messages.HtmlOutputter_Msg_TreeIsLimited)
				.append("</td>");
		artefact.append("</tr>");
	}

	@SuppressWarnings("nls")
	private void renderDataColumns(Context context,
			Writer artefact,
			RefinedStructuredResult structured,
			Column[] columns,
			Object row,
			boolean hasDetailsLink) throws IOException {
		...
		if (hasDetailsLink) {
			...
			for (DetailResultProvider p : structured.getResultMetaData().getDetailResultProviders()) {
				if (p.hasResult(row)) {
					try {
						String link =
								context.addContextResult(p.getLabel(), p.getResult(row, new VoidProgressListener()));
						if (link != null) {
	...
	@SuppressWarnings("nls")
	private void renderColumnValue(Context context,
			Writer artefact,
			RefinedStructuredResult structured,
			Column[] columns,
			Object row,
	...
	/**
	 * Is this new context significantly different from the base context and worth displaying. If they both are simple
	 * contexts and the new id is different and not -1, then display it. If either are IContextObjectSets then they
	...
	private boolean moreContextObjects(IContextObject c1, IContextObject c2) {
		...
		if (c1.getObjectId() != c2.getObjectId() && c2.getObjectId() != -1)
			return true;
		if (!(c1 instanceof IContextObjectSet) && !(c2 instanceof IContextObjectSet))
			return false;
		return true;
	}

	@SuppressWarnings("nls")
	private boolean renderLink(Context context,
			Writer artefact,
			IStructuredResult thing,
			Object row,
			String label,
			Column columns[],
			int columnIndex) throws IOException {
				...
				for (DetailResultProvider provider : detailProvider) {
					if (name.equals(provider.getLabel())) {
						if (provider.hasResult(identifier)) {
							try {
								String l = context.addContextResult(provider.getLabel(),
										provider.getResult(identifier, new VoidProgressListener()));
								if (l != null) {
									writer.append(l);
									done = true;
								}
