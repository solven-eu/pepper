...
/* package */class RenderingInfo implements IOutputter.Context {
	...
	public String addContextResult(String name, IResult result) {
		...
		if (filename == null) {
			/*
			 * Guess the format that might be used. For example, "stacktrace with involved local variables" in leak
			 * suspects TextResult as HTML with a default mode of csv. The file name is decided here, but the true
			 * format is decided later.
			 */
