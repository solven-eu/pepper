...
/* package */class PageSnippets {
	...
	public static void beginPage(final AbstractPart part, HtmlArtefact artefact, String title, String encoding) {
		...
		artefact.append("<link rel=\"stylesheet\" type=\"text/css\" href=\"")
				.append(artefact.getPathToRoot())
				.append("styles.css\">");
		...
		artefact.append("<script src=\"")
				.append(artefact.getPathToRoot())
				.append("code.js\" type=\"text/javascript\"></script>");
		artefact.append("</head><body onload=\"preparepage();\">");

		// Pass the text used for hide / unhide
		artefact.append("<div><input type=\"hidden\" id=\"imageBase\" value=\"")
				.append(artefact.getPathToRoot())
				.append("img/\" title=\"")
		...
		artefact.append("<a class=\"sr-only\" href=\"#content\">")
				.append(Messages.PageSnippets_Label_SkipToMainContent)
				.append("</a>");
		artefact.append("<ul>");
		...
		if (part == null) {
			...
			String id = "content";
			artefact.append("<a href=\"#" + id + "\">")
					.append(HTMLUtils.escapeText(part.spec().getName()))
					.append("</a></li>");
		}
	...
	private static void addCommandLink(AbstractPart part, HtmlArtefact artefact) {
		if (part.getCommand() != null) {
			...
			artefact.append(" <a href=\"")
					.append(QueryObjectLink.forQuery(part.getCommand())) //
					.append("\" title=\"")
					.append(Messages.PageSnippets_Label_OpenInMemoryAnalyzer)
					.append(" ") //
					.append(cmdString)
					.append("\"><img src=\"") //
					.append(artefact.getPathToRoot())
					.append("img/open.gif\" alt=\"")
	...
	public static void heading(HtmlArtefact artefact,
			AbstractPart part,
			int order,
		...
		if (!showHeading) {
			...
			if (isExpandable) {
				...
				artefact.append("<a href=\"#\" onclick=\"hide(this, '")
						.append(id("exp", part)) //
						.append("'); return false;\" title=\"") //
						.append(isExpanded ? Messages.PageSnippets_Label_HideUnhide
								: Messages.PageSnippets_Label_UnhideHide) //
						.append("\"><img src=\"") //
						.append(artefact.getPathToRoot())
						.append(isExpanded ? OPENED : CLOSED) //
						.append("\" alt=\"")
						// .append(isExpanded ? Messages.PageSnippets_Label_HideUnhide :
						// Messages.PageSnippets_Label_UnhideHide) //
						.append("\"></a> ");
			...
			if (status != null)
				artefact.append("<img src=\"")
						.append(artefact.getPathToRoot()) //
						.append("img/")
						.append(status.name().toLowerCase(Locale.ENGLISH) + ".gif\" alt=\"")
		...
		if (!showHeading) {
			...
			if (query.getStatus() != null)
				artefact.append("<img src=\"")
						.append(artefact.getPathToRoot())
						.append("img/")
						.append(query.getStatus().name().toLowerCase(Locale.ENGLISH) + ".gif\" alt=\"")
						.append(query.getStatus().toString())
						.append("\"> ");
	...
	/**
	 * Retrieve the current language of the spec of the part with a " lang=" prefix or nothing if it is the same as the
	 * parent part.
	 *
	 * @param part
	...
	/**
	 * Retrieve the current language of the part or spec with a " lang=" prefix or nothing if it is the same as the
	 * parent part.
	 *
	 * @param part
	 *            Query
